...
class ExecutionEvidence:
    ...
    declared_output_schemas: tuple[str, ...]
...
class ConsolidationLedger:
    ...
    It never mutates the production registry/reflex router. Repeated evidence
    may create a candidate, but production promotion remains regression-gated.
    """

    evidence: list[ExecutionEvidence] = field(default_factory=list)

    def record(self, result: ExecutionResult, state: TaskState, *, declared_inputs: tuple[str, ...], declared_outputs: tuple[str, ...]) -> ExecutionEvidence:
        route_level = state.route_history[-1].level if state.route_history else None
        shape = {
            "capability_id": result.capability_id,
            "input_schemas": tuple(declared_inputs),
            "output_schemas": tuple(declared_outputs),
        ...
        item = ExecutionEvidence(
            ...
            ok=result.ok,
            input_schemas=tuple(declared_inputs),
            declared_output_schemas=tuple(declared_outputs),
            route_level=route_level,
    ...
    def candidate(self, capability_id: str, *, min_successes: int = 3, target_level: str = "L1") -> CapabilityCandidate | None:
        ...
        fingerprint, strongest = max(groups.items(), key=lambda pair: len(pair[1]))
