...
def retrieve_text(index:BibleSearchIndex, text:str, *, limit:int=20)->ReflexResult:
    """RC2: exact/substring evidence wins; fuzzy is only fallback."""
    exact=index.search(SearchQuery(text,mode='text',limit=limit))
    if exact:return ReflexResult('text-retrieval',('exact','normalized'),tuple(exact),1.0,'exact-or-substring textual evidence')
    fuzzy=index.search(SearchQuery(text,mode='fuzzy',limit=limit))
    confidence=fuzzy[0].score if fuzzy else 0.0
    return ReflexResult('text-retrieval',('exact','normalized','bounded-fuzzy'),tuple(fuzzy),confidence,'fuzzy candidates are not facts')
...
def original_language_route(index:BibleSearchIndex, translated_phrase:str, original_units:Iterable, *, language:str)->ReflexResult:
    ...
    This deliberately stops at verse-level co-attestation unless an original
    unit itself carries explicit ``translation_alignment`` analysis evidence.
    """
...
def compare_witnesses(clusters:Iterable[AlignmentCluster], canonical_ref_id:str)->ReflexResult:
    """RC4: compare witnesses without selecting a winner or synthesizing absence."""
...
def resolve_entity(mention:str, entities:Iterable[WorldEntity], *, context_refs:Iterable[str]=())->ReflexResult:
    """RC5: surface mention -> candidates -> context-attested disambiguation."""
...
def geography_claims(place_id:str, claims:Iterable[WorldClaim])->ReflexResult:
    """RC6: keep Scripture-explicit and reconstruction evidence epistemically separate."""
