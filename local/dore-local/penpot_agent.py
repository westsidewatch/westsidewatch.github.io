...
def visual_verify(img,task,brief):
 try:
  m=_ollama([{'role':'user','content':f'Judge the ACTUAL Penpot render. Task: {task}\nBrief: {brief}\nReturn strict JSON: verdict PASS/FAIL, problems, strengths, next_correction. Blank, duplicated, cluttered, or test-looking output is FAIL.','images':[img]}],VISION_MODEL);s=m.get('content','');return json.loads(s[s.index('{'):s.rindex('}')+1])
...
def _agent_code(task,brief,bid,docs,clean=False):
 mode='''CLEAN-SLATE MODE. Create ONE fresh board named "00 — Westside Watch Homepage — CLEAN START" on penpot.root. Do not reuse or copy old homepage boards. The new board must be the only artifact you work on in this task. Build the first coherent editorial homepage composition from scratch. Do not create swatches, demos, test rectangles, or duplicate boards.''' if clean else f'''EXISTING-BOARD MODE. Locate and modify only existing board id {bid}. Never create a new board.'''
...
Use only official APIs evidenced below. Preserve native editable Penpot layers. Implement the design materially. At the end console.log('DORE_BOARD_ID='+board.id), console.log child count, and return a short success string. TASK:\n{task}\nBRIEF:\n{brief}\nOFFICIAL API GROUNDING:\n{docs[:16000]}'''
...
def run_task(task,design_brief):
 ...
 clean='CLEAN_START' in (task or '')
 bid=None if clean else _board_id(task)
 ...
 try:code=_agent_code(task,design_brief,bid,docs,clean)
 ...
 mut=call_tool('execute_code',{'code':code});actual=_extract_board_id(mut) or bid;trace.append({'tool':'execute_code','ok':bool(mut.get('ok')),'mutation':True,'phase':'clean-start-rebuild' if clean else 'task-aware-existing-board-mutation','board_id':actual,'text':_text_from_result(mut)[:3500]})
 if not mut.get('ok') or not actual:return {'ok':False,'verified':False,'mutated':False,'error':'penpot_task_mutation_failed','board_id':actual,'trace':trace}
