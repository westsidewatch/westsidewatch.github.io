...
def _persist_evidence(design,task_id,checks):
 ...
 for candidate in candidates:
  ...
  if src.is_file():
   dst=out/f'{cid}.png';shutil.copy2(src,dst);raster_files.append({'candidate_id':cid,'file':dst.name,'sha256':raster.get('sha256'),'width':raster.get('width'),'height':raster.get('height'),'byte_size':raster.get('byte_size'),'used_asset_refs':candidate.get('used_asset_refs') or []})
 critic=design.get('critic') or {}
 evidence={
  'schema':'dore.living-water-bloom.evidence.v3','task_id':task_id,'experiment_id':design.get('experiment_id'),'checks':checks,
...
def main():
 with tempfile.TemporaryDirectory(prefix='dore-living-water-real-') as td:
  ...
  arsenal=design.get('arsenal') or {};mount=arsenal.get('mount') or {};life=mount.get('lifecycle') or [];by_cap={str(row.get('capability')):row for row in life};loadout=((arsenal.get('plan') or {}).get('loadout') or {});selected=set(by_cap);consumed=set(arsenal.get('inputs_consumed_by_generation') or [])
  image_row=by_cap.get('image.generate') or {};visual_assets=arsenal.get('visual_assets') or [];visual_refs={str(a.get('asset_ref')) for a in visual_assets if a.get('asset_ref')};used_refs=set(arsenal.get('visual_asset_refs_used') or [])
  if visual_assets:
   image_truth=bool(image_row.get('state')=='consumed' and image_row.get('consumed') is True and image_row.get('consumed_by')=='candidate-raster-material' and 'image.generate' in consumed and visual_refs and visual_refs.intersection(used_refs) and all(set(c.get('used_asset_refs') or []).intersection(visual_refs) for c in candidates))
  else:
   image_truth=bool(image_row.get('state') in {'executed','executed-no-output','execution-failed','selected-not-mounted'} and image_row.get('consumed') is False and 'image.generate' not in consumed and not used_refs)
  checks={'canonical':result.get('capability_id')=='design.intelligence','universal':((result.get('core_route') or {}).get('execution_authority')=='a2a_execution_plane'),'pass':execution.get('execution_status')=='PASS' and proof.get('completion_evidence') is True,'real':design.get('provider')=='dore-local' and str(design.get('model') or '') not in {'','fixture','local-default'},'pixels':len(candidates)==2 and all(r.get('real_browser_render') and r.get('sha256') for r in rasters),'judges':len((design.get('critic') or {}).get('votes') or [])==2,'immutable':design.get('canonical_workspace_mutated') is False,'not_promoted':design.get('production_promoted') is False,'bloom_contract':design.get('experiment_contract_applied') is True and design.get('experiment_id')=='living-water-bloom','divergent':len(design.get('divergence_domains') or [])>=4,'old_design_not_authority':design.get('historical_design_authority') is False,'arsenal_roles':not (loadout.get('unresolved_roles') or []),'arsenal_no_self_recursion':mount.get('no_self_recursion') is True,'arsenal_complementary':{'design.intelligence','image.generate','westside.context','knowledge.recall'}.issubset(selected),'arsenal_identity_consumed':(by_cap.get('westside.context') or {}).get('state')=='consumed' and 'westside.context' in consumed,'arsenal_image_evidence_truthful':image_truth}
  ...
  assert (evidence.get('arsenal') or {}).get('mount'),'persisted_arsenal_mount_required'
  print(json.dumps({'ok':True,'code':'LIVING_WATER_BLOOM_REAL_PASS','checks':checks,'task_id':task_id,'evidence_dir':str(out),'winner':evidence.get('winner'),'failure_domains':[x.get('domain') for x in evidence.get('loser_failures') or []],'memory_admitted':evidence.get('memory_admitted'),'arsenal_states':{cid:row.get('state') for cid,row in by_cap.items()},'visual_asset_refs':sorted(visual_refs),'visual_asset_refs_used':sorted(used_refs),'visual_asset_error':arsenal.get('visual_asset_error')},ensure_ascii=False))
