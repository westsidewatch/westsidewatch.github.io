...
def _git_publish(msg):
 ...
 try:
  ...
  for attempt in range(1,4):
   _run(['git','fetch','origin','main'],60);td=Path(tempfile.mkdtemp(prefix='dore-mail-'))
   try:
    add=_run(['git','worktree','add','--detach',str(td),'origin/main'],60)
    if add.returncode:last=add.stderr[-400:];continue
...
def flush_outbox():
 ...
 return results
def send_to_chatgpt(subject:str,body:str,*,requires_reply=False,priority='normal',related_goal=None,evidence_refs=None,thread_id=None,message_id=None,metadata=None):
