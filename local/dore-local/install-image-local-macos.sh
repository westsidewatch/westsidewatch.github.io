...
PY
  ...
  if [ -n "$MODEL" ]; then
    cat > "$CONFIG" <<EOF
{"schema":"dore.image.resident.v1","endpoint":"http://127.0.0.1:8188","model":$(printf '%s' "$MODEL" | "$PY" -c 'import json,sys; print(json.dumps(sys.stdin.read()))'),"template":"dore-image/workflows/editorial-basic.json","output_dir":"dore-image/generated"}
EOF
  else
    rm -f "$CONFIG"
  fi
...
for _ in $(seq 1 20); do
  H="$(curl -fsS --max-time 2 http://127.0.0.1:8790/health 2>/dev/null || true)"
  if printf '%s' "$H" | grep -q '"renderer": true'; then echo "$H"; echo DORE_IMAGE_LOCAL_PASS; exit 0; fi
  sleep 1
done
echo "ERROR: Doré Image Local health failed" >&2; cat "$DORE/logs/image-local.err.log" >&2 2>/dev/null || true; exit 22
