...
def _candidate_roots(repo_root:Path,dore_root:Path):
    return [p for p in (repo_root,dore_root/'archive') if p.exists()]
...
def evidence_pack(hits,max_files=10,max_chars_each=4500):
    ...
    for h in hits[:max_files]:
        ...
        q=h.get('query') or ''; pos=text.lower().find(q.lower()) if q else -1
        start=max(0,pos-1500) if pos>=0 else 0; excerpt=text[start:start+max_chars_each]
        pack.append({'source_ref':str(p),'matched_query':q,'excerpt':excerpt})
    return pack
...
def synthesize(model_call,gate,pack):
    ...
    return _call_json(model_call,prompt)

def challenge(model_call,gate,pack,synthesis):
    prompt='''Act as an adversarial research examiner independent of the synthesis. Using the evidence, attack unsupported leaps, circular provenance, inherited-memory self-authentication, missing counterevidence, and acceptance criteria not actually demonstrated. Return strict JSON: objections (array), unsupported_claims (array), acceptance_met (array), acceptance_missing (array), verdict_reason. Do not award PASS.\nGATE:\n'''+json.dumps(gate,ensure_ascii=False)+'\nEVIDENCE:\n'+json.dumps(pack,ensure_ascii=False)+'\nSYNTHESIS:\n'+json.dumps(synthesis,ensure_ascii=False)
    return _call_json(model_call,prompt)

def blind_assessment(model_call,gate,pack):
    q_prompt='''Create 3 difficult blind-test questions from this evidence pack. They must test relational understanding, provenance, and ability to distinguish claim from evidence; avoid trivia and avoid embedding answers. Return strict JSON {questions:[...]} only.\n'''+json.dumps(pack,ensure_ascii=False)
    questions=_call_json(model_call,q_prompt).get('questions') or []
    a_prompt='''Answer these questions without seeing source material. Be concise and explicitly mark uncertainty. Return strict JSON {answers:[...]} only.\nQUESTIONS:\n'''+json.dumps(questions,ensure_ascii=False)
    answers=_call_json(model_call,a_prompt).get('answers') or []
    g_prompt='''Grade the blind answers against the evidence pack. Return strict JSON with score from 0.0 to 1.0, errors (array), unsupported (array), provenance_failures (array), and reason. High score requires correct relationships and provenance, not plausible prose.\nQUESTIONS:\n'''+json.dumps(questions,ensure_ascii=False)+'\nANSWERS:\n'+json.dumps(answers,ensure_ascii=False)+'\nEVIDENCE:\n'+json.dumps(pack,ensure_ascii=False)
    grade=_call_json(model_call,g_prompt)
    return {'questions':questions,'answers':answers,'grade':grade}

def _structural_research_method_pass(gate,pack,synthesis,critic):
    claims=synthesis.get('claims') if isinstance(synthesis,dict) else None
    refs=[]
    for c in claims or []:
...
def _autonomous_learning_pass(gate,pack,synthesis,critic,blind):
    grade=blind.get('grade') or {}; score=float(grade.get('score') or 0)
    unsupported=grade.get('unsupported') or []; prov=grade.get('provenance_failures') or []
    missing=critic.get('acceptance_missing') or []
    return len(pack)>=int(gate.get('min_evidence') or 1) and score>=0.85 and not unsupported and not prov and len(missing)<=1

def _find_learning_event(conn,domain,stage):
    return conn.execute("SELECT id FROM dore_learning_events WHERE domain=? AND COALESCE(stage,'')=COALESCE(?, '') AND epistemic_state!='rejected' ORDER BY updated_at DESC LIMIT 1",(domain,stage)).fetchone()
...
def execute_gate(conn,repo_root:Path,dore_root:Path,gate,model_call=None):
    ...
    if prev and prev['evidence_hash']==digest and prev['phase'] in {'blocked','completed'}:
        return {'gate_id':gate['id'],'phase':'stagnant','attempt':prev['attempt'],'evidence_count':len(hits)}
    rid='run_'+_id(gate['id'],str(attempt),digest or 'none',now()); ref=f'autonomous-run:{rid}'; pack=evidence_pack(hits)
    result={'gate_id':gate['id'],'domain':gate['domain'],'stage':gate.get('stage'),'evidence':hits,'acceptance':gate.get('acceptance') or [],'next_action':gate.get('next_action')}
    phase='blocked'; verified=False
    if len(pack)>=int(gate.get('min_evidence') or 1) and model_call:
        synthesis=synthesize(model_call,gate,pack); critic=challenge(model_call,gate,pack,synthesis); result.update({'synthesis':synthesis,'critic':critic})
    ...
    result['verified_by_executor']=verified
    t=now(); final_phase='completed' if verified else phase
...
def run_cycle(conn,repo_root:Path,dore_root:Path,max_gates=1,model_call=None):
    ensure_schema(conn); gates=load_gates(Path(__file__).resolve().parent); p=plan(learning_status(conn),gates); ready=p.get('ready') or []; executed=[]
    for item in ready[:max_gates]: executed.append(execute_gate(conn,repo_root,dore_root,next(g for g in gates if g['id']==item['id']),model_call))
    return {'ok':True,'policy':'bounded-autonomous-learning-v2','time_is_gate':False,'executed':executed,'planner':p}
