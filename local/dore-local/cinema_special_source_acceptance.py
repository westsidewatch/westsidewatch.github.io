...
assert index.index('special-resource-policy.js') < index.index('resource-graph.js') < index.index('cinema.js')
assert 'goodtv/' in index
assert 'cinema:video:goodtv:holy-spirit-power-workplace-testimony' in policy
assert 'window.fetch=' not in policy and 'nativeFetch' not in policy
assert 'ParadiseCinemaSpecialResources' in graph and '.filter(resourcePayload.items)' in graph
