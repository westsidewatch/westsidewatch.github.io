#!/usr/bin/env python3
"""Framesmith MCP real-work trial. Tool names follow upstream Framesmith 2.0 docs/source."""
...
def main():
 m=None;tr=[]
 try:
  m=MCP();m.req('initialize',{'protocolVersion':'2025-03-26','capabilities':{},'clientInfo':{'name':'dore','version':'0.2'}},30);m.notify('notifications/initialized')
  listed=m.req('tools/list',{},30);names=[x['name'] for x in listed.get('tools',[])];tr.append(['tools',names])
  required=['init','canvas_create','generate_design_system','list_structures','apply_structure','batch_design','screenshot','canvas_evaluate']
  missing=[x for x in required if x not in names]
  if missing:raise RuntimeError('actual_upstream_tools_missing:'+','.join(missing))
  state=unwrap(m.tool('init',{'dir':str(REPO),'workspaceName':'Doré Design','projects':['Foundations','Westside Watch']}));tr.append(['init',state])
  plist=unwrap(m.tool('project_list',{}));tr.append(['projects',plist]);projects=(plist.get('projects') if isinstance(plist,dict) else None) or []
  ...
  # Upstream batch_design accepts its documented line-separated operation language.
  ...
  out={'ok':True,'provider':'framesmith','artifact':cid,'viewerUrl':c.get('viewerUrl'),'structured_editable':True,'visible':True,'render':shot1,'second_edit':True,'second_render':shot2,'evaluation':ev,'trace':tr}
