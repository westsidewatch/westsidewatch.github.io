#!/usr/bin/env python3
"""Fixed offline base-vs-micro64 held-out evaluation with multilingual degeneration guard."""
from __future__ import annotations
import json, os, shutil, subprocess, sys
from pathlib import Path
CACHE_ROOT=Path.home()/"Library"/"Caches"/"Dore"/"theology-training"; HF_HOME=CACHE_ROOT/"hf"; PY=CACHE_ROOT/"venv"/"bin"/"python"; MODEL="mlx-community/gemma-4-e4b-it-4bit"
...
def degenerate(text):
    toks=[t.strip(".,!?;:*_`'\"()[]{}-").casefold() for t in text.split() if t.strip()]; run=1; worst=1
...
def ensure_bundle():
    ...
    shutil.rmtree(BUNDLE,ignore_errors=True); BUNDLE.mkdir(parents=True); shutil.copyfile(ADAPTER_CONFIG,BUNDLE/"adapter_config.json"); shutil.copyfile(ADAPTER_FILE,BUNDLE/"adapters.safetensors")
def worker(mode):
    from mlx_vlm import generate, load
    from mlx_vlm.prompt_utils import apply_chat_template
    model,processor=load(MODEL,**({"adapter_path":str(BUNDLE)} if mode=="adapter" else {})); rows=[]
    for c in CASES:
        p=apply_chat_template(processor,model.config,c["prompt"],num_images=0); out=generate(model,processor,prompt=p,max_tokens=160,temperature=0.0,verbose=False); text=out if isinstance(out,str) else getattr(out,"text",str(out)); hit,total=score(text,c["groups"]); rows.append({"id":c["id"],"prompt":c["prompt"],"output":text,"concept_hits":hit,"concept_total":total,"degenerate":degenerate(text)})
    print(json.dumps({"mode":mode,"rows":rows},ensure_ascii=False))
def run_mode(mode):
    env=os.environ.copy(); env["HF_HOME"]=str(HF_HOME); env["HF_HUB_CACHE"]=str(HF_HOME/"hub"); env["HF_HUB_OFFLINE"]="1"; env["TRANSFORMERS_OFFLINE"]="1"; p=subprocess.run([str(PY),str(Path(__file__).resolve()),"--worker",mode],text=True,capture_output=True,timeout=1500,env=env)
    if p.returncode!=0: raise RuntimeError(f"{mode}_worker_failed:{p.returncode}:{p.stderr[-3000:]}")
    return json.loads(p.stdout)
def aggregate(r):
    hit=sum(x["concept_hits"] for x in r["rows"]); total=sum(x["concept_total"] for x in r["rows"]); deg=sum(1 for x in r["rows"] if x["degenerate"]); return {"hits":hit,"total":total,"ratio":round(hit/total,4) if total else 0.0,"degenerate_cases":deg}
def main():
