...
def _design_intelligence(args:dict[str,Any])->dict[str,Any]:
 ...
 try:
  ...
  if isinstance(result,dict):
   result=dict(result);result.setdefault('status','completed' if result.get('ok') else 'failed');result.setdefault('capability','design.intelligence')
  return result
 finally:
  if inserted:
...
def _bible_query_plan(args:dict[str,Any])->dict[str,Any]:
 ...
 return {"ok":True,"status":"completed","capability":"bible.query-plan","plan":plan.to_dict(),"dore_identity":True,"large_model_invoked":False}
...
def _fuzzy_search(args:dict[str,Any],caller_product:str|None=None)->dict[str,Any]:
 ...
 def search_documents(text:str,*,semantic:bool,deep:bool,limit:int)->dict[str,Any]:return qmd_search(text,qmd_config,semantic=semantic,deep=deep,limit=limit)
 def recall_current(text:str,*,mode:str)->dict[str,Any]:
  if not longmemory_available():return {"ok":False,"authority":False,"error":"memory_unavailable"}
 ...
 return {"ok":internal.get("ok",False),"status":"completed" if internal.get("ok",False) else "failed","capability":"context.fuzzy-search","query":query,"results":internal.get("results",[]),"retrieval":{"lexical":plan.lexical,"semantic":plan.semantic,"deep":plan.deep,"memory_recall":plan.recall_memory,"reason":plan.reason},"bible_plan":bible_plan.to_dict(),"capability_runtime":{"identity":"dore","execution_policy":"lowest-sufficient-capability","execution_level":bible_plan.execution_level,"model_invoked":False,"escalation_available":bible_plan.reasoning_allowed or bible_plan.semantic_allowed},"context_policy":internal.get("context_policy"),"search_scope":"production" if qmd_config.collection==PRODUCTION_COLLECTION else "requested-collection","authority":False,"large_model_invoked":False,"core_route":{"capability":"context.fuzzy-search","caller_product":caller_product,"provider":"dore-search","transport":"core-adapter","identity":"dore"}}
...
def _knowledge_recall(args:dict[str,Any])->dict[str,Any]:
 ...
 db=Path(os.environ.get("DORE_LONGMEMORY_DB") or (Path.home()/".dore"/"knowledge"/"longmemory.db"));project=str(args.get("project") or os.environ.get("DORE_LONGMEMORY_PROJECT") or "dore");mode=str(args.get("mode") or "strict");return longmemory_recall(query,LongMemoryConfig(db=db,project=project),mode=mode)
