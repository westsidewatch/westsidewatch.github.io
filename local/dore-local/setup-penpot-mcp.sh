...
# Penpot 2.13.3 upstream reuses one McpServer across HTTP/SSE transports.
# Patch it so every new MCP transport gets its own server instance, while tools share the same Penpot bridge.
...
must('''                    await this.server.connect(transport);''','''                    const freshServer = new McpServer(
                        { name: "penpot-mcp-server", version: "1.0.0" },
                        { instructions: this.getInitialInstructions() }
                    );
                    this.registerTools(freshServer);
                    await freshServer.connect(transport);''','streamable connect');
must('''                await this.server.connect(transport);''','''                const freshServer = new McpServer(
                    { name: "penpot-mcp-server", version: "1.0.0" },
                    { instructions: this.getInitialInstructions() }
                );
                this.registerTools(freshServer);
                await freshServer.connect(transport);''','sse connect');
fs.writeFileSync(file,s);
NODE
(
  cd "$PKG_DIR/packages/server"
  "$PNPM" run build
)
