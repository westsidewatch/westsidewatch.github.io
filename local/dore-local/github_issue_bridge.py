...
PROTOCOL="dore.a2a/1"; TITLE_PREFIX="[DORÉ A2A]"; CAP_RE=re.compile(r"^[A-Za-z0-9_.-]{1,128}$"); MAX_COMMENT_BYTES=60000
...
def post_comment(repo,issue_number,token,payload):
    ...
    if len(raw)>MAX_COMMENT_BYTES:
        payload=fail("result_too_large"); marker="[DORÉ_LOCAL_RESULT "+json.dumps(payload,separators=(",",":"))+"]"; raw=json.dumps({"body":marker}).encode()
    req=urllib.request.Request(f"https://api.github.com/repos/{repo}/issues/{issue_number}/comments",data=raw,method="POST",headers={"Authorization":f"Bearer {token}","Accept":"application/vnd.github+json","X-GitHub-Api-Version":"2022-11-28","Content-Type":"application/json","User-Agent":"dore-a2a-github-relay"})
    with urllib.request.urlopen(req,timeout=20) as r:
        if r.status not in (200,201): raise RuntimeError(f"comment_http_{r.status}")
def main():
    repo=os.environ.get("GITHUB_REPOSITORY",""); actor=os.environ.get("GITHUB_ACTOR",""); title=os.environ.get("DORE_ISSUE_TITLE",""); body=os.environ.get("DORE_ISSUE_BODY",""); issue_number=os.environ.get("DORE_ISSUE_NUMBER",""); token=os.environ.get("GITHUB_TOKEN",""); owner=repo.split("/",1)[0] if "/" in repo else ""
    try:
        if not repo or not issue_number or not token: raise ValueError("missing_github_context")
        if actor!=owner: raise PermissionError("actor_not_repository_owner")
        if not title.startswith(TITLE_PREFIX): raise PermissionError("invalid_title_prefix")
        command=json.loads(body)
        if not isinstance(command,dict): raise ValueError("body_must_be_json_object")
        ...
        timeout=CAPABILITY_TIMEOUTS.get(capability,360.0 if capability in LONG_CAPABILITIES else 15.0); rpc=call("dore.call",{"capability":capability,"args":args},timeout=timeout)
    ...
    if repo and issue_number and token: post_comment(repo,issue_number,token,result)
