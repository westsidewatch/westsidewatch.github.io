...
Delivery proves that a message arrived; this plane proves that work was claimed,
executed, produced an artifact, verified, and only then completed. Lease expiry is
never treated as terminal failure: it creates explicit recoverability state.
"""
...
def read(task_id):
 p=_task_path(task_id);return json.loads(p.read_text(encoding='utf-8')) if p.exists() else None

def register(message,delivery=None):
 task_id=str(message.get('message_id') or '')
 if not task_id:raise ValueError('task_id_required')
 d=delivery or message.get('_delivery') or {};digest=d.get('content_sha256') or _digest({k:v for k,v in message.items() if k!='_delivery'});existing=read(task_id)
 if existing:
...
def heartbeat(task_id,consumer=None,lease_seconds=DEFAULT_LEASE_SECONDS):
 ...
 if not _lease_live(lease):return {'ok':False,'code':'LEASE_EXPIRED','task':t}
 lease['expires_at']=(datetime.now(timezone.utc)+timedelta(seconds=max(30,int(lease_seconds)))).isoformat();lease['heartbeat_at']=now();t['lease']=lease;_atomic(_task_path(task_id),t);_append({'at':now(),'task_id':task_id,'event':'LEASE_HEARTBEAT','owner':owner,'expires_at':lease['expires_at']});return {'ok':True,'code':'LEASE_RENEWED','task':t}
...
def complete(task_id,result=None,*,consumer=None):
 ...
 lease=t.get('lease') or {}
 if lease.get('owner')!=owner or not _lease_live(lease):return {'ok':False,'code':'LEASE_REQUIRED','task':t}
 if t['status']!='VERIFIED' or not (t.get('verification') or {}).get('ok') or not t.get('artifact'):return {'ok':False,'code':'VERIFIED_ARTIFACT_REQUIRED','task':t}
...
def status(task_id):
 t=read(task_id)
 if not t:return {'ok':False,'code':'TASK_NOT_FOUND','task_id':task_id}
 rec=recoverability(task_id)
 return {'ok':True,'task':t,'recovery_state':rec.get('state'),'recovery':{k:v for k,v in rec.items() if k not in {'ok','task'}},'completion_evidence':bool(t.get('status')=='PASS' and t.get('artifact') and (t.get('verification') or {}).get('ok'))}
