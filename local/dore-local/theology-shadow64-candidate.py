...
spec=importlib.util.spec_from_file_location("shadow64_base",HERE/"theology-shadow64-acceptance.py")
...
m.BUNDLE=m.CACHE_ROOT/"runs"/"n64-recovery"/"shadow-candidate-bundle"
def candidate_run_mode(mode):
    ...
    p=subprocess.run([str(m.PY),str(Path(__file__).resolve()),"--worker",mode],text=True,capture_output=True,timeout=1800,env=env)
