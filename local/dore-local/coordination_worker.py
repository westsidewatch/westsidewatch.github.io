...
def run_acceptance_tasks(prev):
 ...
 for p in sorted(TASKS.glob('*.json')):
  ...
  if t.get('kind')!='conversation_acceptance_v1': continue
  script=ROOT/'local/dore-local/conversation_acceptance.py'
  try:
   cp=subprocess.run([str(Path(os.environ.get('PYTHON','python3'))),str(script)],cwd=ROOT,text=True,capture_output=True,timeout=900)
   body=(cp.stdout or cp.stderr or '').strip()[-12000:]
   ...
   prev['autonomous_tasks_processed']=sorted(done); prev['checked_at']=datetime.now(timezone.utc).isoformat(); save(prev); flush_outbox(); return done,True
  except Exception as e:
   prev['last_error']='acceptance_task:'+type(e).__name__; prev['checked_at']=datetime.now(timezone.utc).isoformat(); save(prev); return done,True
 return done,False
def main():
 flush_outbox()
 ...
 try:decision=parse(ask(prompt))
  ...
  prev.update({'packet_sha256':prev.get('packet_sha256'),'repo_inbox_processed':sorted(processed),'checked_at':datetime.now(timezone.utc).isoformat(),'last_error':'model_or_parse:'+type(e).__name__}); save(prev); return 1
 if current and current.get('requires_reply') and not decision.get('send'):
  prev.update({'packet_sha256':prev.get('packet_sha256'),'repo_inbox_processed':sorted(processed),'checked_at':datetime.now(timezone.utc).isoformat(),'last_error':'required_reply_not_generated'}); save(prev); return 2
