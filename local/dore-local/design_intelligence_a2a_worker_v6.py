...
import design_intelligence_a2a_worker as legacy
import design_rejection_enforcement as enforcement


def _guardrails(payload):
    pack=payload.get('preference_pack') or {}
    values=pack.get('rejection_guardrails') or []
    return [g for g in values if isinstance(g,dict) and g.get('failure_domain')]


def _generate(ollama,payload,base,nodes,attempt):
    guardrails=_guardrails(payload)
    system=(
      'You are Doré Design Core. Generate exactly two materially different, brand-faithful executable patches. '
      'Return JSON only with variants [A,B]. Each variant must contain id, direction, patch, and risk_domains. '
      'risk_domains must truthfully list any known failure-domain risk the proposal may reproduce. '
      'patch schema is dore.design.candidate-patch.v1; allowed ops: move, resize, font_size, text_align. '
      'The rejection guardrails are bounded negative precedent: do not repeat them. Do not choose a winner.'
    )
    ...
    if not isinstance(variants,list) or len(variants)!=2 or [str(v.get('id')) for v in variants]!=['A','B']:
        raise ValueError('exactly_A_B_variants_required')
    candidates=[]
    for v in variants:
        item=legacy.sandbox.materialize(base,v.get('patch'),v['id'])
        item['risk_domains']=[str(x) for x in (v.get('risk_domains') or [])]
...
def _model(payload):
    ...
    for attempt in range(enforcement.MAX_REGENERATIONS+1):
        variants,candidates=_generate(ollama,payload,base,nodes,attempt)
        enforcement_result=enforcement.enforce(candidates,guardrails)
        if enforcement_result['all_admitted']:
            break
        if attempt>=enforcement.MAX_REGENERATIONS:
            raise RuntimeError('rejection_enforcement_exhausted')
