#!/usr/bin/env python3
"""Doré Research Executor v0.2.

RESEARCH_QUEUED becomes real resource discovery: verified local knowledge and
failure memory first, then curated authoritative sources and available public
search clients. If self-research has no relevant evidence, publish a durable
peer-research request to ChatGPT. Discovery is never confused with verification.
"""
from __future__ import annotations
import json, os, re, shutil, subprocess, sys
from datetime import datetime, timezone
from pathlib import Path
HOME=Path(os.environ.get('DORE_LOCAL_HOME',Path.home()/'.dore')).expanduser();ROOT=Path(os.environ.get('DORE_REPO_ROOT',Path.home()/'westsidewatch.github.io')).expanduser();RESEARCH=HOME/'coordination'/'research';KNOWLEDGE=ROOT/'dore-design'/'knowledge-lab';CATALOG=KNOWLEDGE/'resources'/'source-catalog.json'
...
def terms(text):
    ...
    return out[:12]
def local_search(query_terms,limit=18):
    hits=[];roots=[KNOWLEDGE,ROOT/'local'/'dore-local',HOME/'coordination'/'learning']
    for base in roots:
        ...
        for p in base.rglob('*'):
            ...
            if score:
                sample=next((line.strip()[:500] for line in text.splitlines() if any(t in line.lower() for t in query_terms)),'')
    ...
    try:
        ...
        for row in find(' '.join(query_terms),limit=8):hits.append({'kind':'failure-memory','score':20,'record':row})
...
def catalog_search(query_terms):
    ...
    for s in data.get('sources') or []:
        hay=' '.join([str(s.get('id','')),str(s.get('upstream',''))]+list(s.get('topics') or [])).lower();score=sum(1 for t in query_terms if t in hay)
        if score:out.append({**s,'score':score,'kind':'catalog-'+str(s.get('kind','source'))})
    return sorted(out,key=lambda x:-x['score'])
def external_search(query_terms):
    q=' '.join(query_terms[:6]) or 'agent research';probes=[];candidates=[];gh=shutil.which('gh')
    if gh:
...
def execute(job_path):
    ...
    text=' '.join([str(job.get('question') or ''),str(job.get('failure_fingerprint') or '')]);qs=terms(text);local=local_search(qs);catalog=catalog_search(qs);external,probes=external_search(qs)
    artifact={'schema':'dore.knowledge-artifact.v1','knowledge_id':'knowledge-'+job['research_id'],'research_id':job['research_id'],'created_at':now(),'discovered_by':'dore-research-executor','query_terms':qs,'sources':{'local':local,'catalog':catalog,'external':external},'tool_probes':probes,'provenance_preserved':True,'reuse_before_rebuild':True,'parent_goal':job.get('parent_goal')}
    evidence_count=len(local)+len(catalog)+len(external);artifact['evidence_count']=evidence_count
    if evidence_count:
        artifact.update({'lesson':'Relevant prior knowledge/resources were found. Form a falsifiable hypothesis from these sources and run the parent-specific minimal experiment before promotion.','hypothesis_status':'CANDIDATES_FOUND','experiment_required':True})
        try:
        ...
        job['state']='KNOWLEDGE_RETURNED';job['knowledge_artifact']=artifact;job['updated_at']=now();atomic_json(p,job);return {'ok':True,'state':'KNOWLEDGE_RETURNED','research_id':job['research_id'],'knowledge_artifact':artifact}
    peer=peer_escalate(job,artifact);job['peer_research']=peer;job['knowledge_artifact']=artifact;job['state']='PEER_RESEARCH_QUEUED' if peer.get('queued') else 'RESEARCH_BLOCKED';job['updated_at']=now();atomic_json(p,job);return {'ok':False,'state':job['state'],'research_id':job['research_id'],'peer_research':peer,'knowledge_artifact':artifact}
