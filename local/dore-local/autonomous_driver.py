#!/usr/bin/env python3
"""Doré Autonomous Driver v0.3 — real A2A <-> Storybook experiment driver."""
from __future__ import annotations
import json, os, re, shutil, subprocess, sys
from datetime import datetime, timezone
from pathlib import Path
HOME=Path(os.environ.get('DORE_LOCAL_HOME',Path.home()/'.dore')).expanduser();ROOT=Path(os.environ.get('DORE_REPO_ROOT',Path.home()/'westsidewatch.github.io')).expanduser();LEARNING=HOME/'coordination'/'learning';EVIDENCE=ROOT/'dore-design'/'knowledge-lab'/'evidence';SKILLS=ROOT/'dore-design'/'knowledge-lab'/'skills';STORYBOOK=ROOT/'dore-design'/'knowledge-lab'/'storybook';MANUAL=ROOT/'dore-design'/'knowledge-lab'/'tools'/'research-bridge-v0.1.md';COMMON_BIN=['/opt/homebrew/bin','/usr/local/bin',str(Path.home()/'.nvm/current/bin')]
...
def run(argv,cwd=ROOT,timeout=300):
 ...
 argv[0]=resolved
 try:
  cp=subprocess.run(argv,cwd=str(cwd),text=True,capture_output=True,timeout=timeout);return {'argv':argv,'cwd':str(cwd),'returncode':cp.returncode,'stdout':(cp.stdout or '')[-12000:],'stderr':(cp.stderr or '')[-12000:]}
...
def summarize_artifact(a):
 ...
 return {'knowledge_id':a.get('knowledge_id'),'research_id':a.get('research_id'),'source_count':sum(len(v or []) for v in sources.values() if isinstance(v,list)),'sources':names[:12],'provenance_preserved':bool(a.get('provenance_preserved')),'lesson':a.get('lesson'),'hypothesis_status':a.get('hypothesis_status')}
def drive(msg):
 mid=str(msg.get('message_id') or 'autonomous-driver');task=msg.get('task') if isinstance(msg.get('task'),dict) else {};parent=str(task.get('parent_source_message_id') or 'new-westside-storybook-real-loop-2');goal=str(task.get('parent_goal') or msg.get('related_goal') or 'New Westside visual construction');knowledge_artifact=task.get('knowledge_artifact') if isinstance(task.get('knowledge_artifact'),dict) else None
 record={'driver':'dore.autonomous-driver.v0.3','message_id':mid,'parent_message_id':parent,'parent_goal':goal,'project_loop':task.get('project_loop') or 'A2A <-> Storybook','started_at':now(),'states':[],'parent_goal_preserved':True,'research_job':task.get('research_job')}
 ...
 state('GOAL',manual=str(MANUAL),trigger=task.get('trigger'))
...
if __name__=='__main__':
 ...
 except Exception as e:out={'ok':False,'driver':'dore.autonomous-driver.v0.3','error':'driver_uncaught_exception','detail':repr(e),'information_gain':True}
