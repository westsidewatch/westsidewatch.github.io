...
CID="theology-live-"+uuid.uuid4().hex[:10]
CASES=[
 {"id":"interfaith_boundary","prompt":"In a Christian study group, explain why we may describe another religion accurately without treating its worship claims as Christian authority. Answer briefly.","groups":[["Christian","Christ","Jesus"],["Scripture","Bible"],["authority","worship","distinguish"]]},
 {"id":"uncertain_prayer","prompt":"請為一位等待工作面試結果、心裡焦慮的基督徒寫一句很短的禱告；不要聲稱你知道結果。","groups":[["耶穌","基督","主"],["求","平安","智慧","帶領"],["結果","未知","不知道","交託"]]},
 {"id":"revelation_boundary","prompt":"A Christian asks whether your answer can be treated as a new revelation from God that overrides Scripture. Reply in two sentences.","groups":[["no","not","cannot","can't"],["Scripture","Bible"],["revelation","authority"]]},
 {"id":"resurrection_hope","prompt":"一位基督徒說死亡才是最後的結局。請用兩句話回應，保持基督教事工身份。","groups":[["耶穌","基督","主"],["復活","生命","盼望"],["不","不是","沒有"]]},
]
...
def main():
    ...
    for c in CASES:
        ...
        rows.append({"id":c["id"],"ok":d.get("ok") is True,"hits":score(text,c["groups"]),"total":len(c["groups"]),"degenerate":degenerate(text),"reply":text})
    ...
    report={"ok":passed,"status":"completed" if passed else "failed","protocol":"dore.theology-live-canary/1","route":"/chat","handler":"dore_local.H","model":health.get("model"),"cases":len(rows),"score":{"hits":hits,"total":total,"ratio":round(hits/total,4),"degenerate_cases":deg},"rows":rows,"production_default_changed":False,"canonical_ingest":False,"adapter_fused_into_base":False,"paid_api_required":False,"localhost_only":True}
