...
CID_PREFIX="theology-live-"+uuid.uuid4().hex[:10]
CASES=[
...
def main():
    ...
    report={"ok":passed,"status":"completed" if passed else "failed","protocol":"dore.theology-live-canary/2","route":"/chat","handler":"dore_local.H","model":health.get("model"),"cases":len(rows),"score":{"hits":hits,"total":total,"ratio":round(hits/total,4),"degenerate_cases":deg},"rows":rows,"production_default_changed":False,"canonical_ingest":False,"adapter_fused_into_base":False,"paid_api_required":False,"localhost_only":True,"bilingual_scoring":True}
