...
def main():
    ...
    prov=editorial_visual_provider.provenance()
    for eid in TARGETS:
        item=items.get(eid)
        ...
        if byte_ev.get('uri') != item.get('image') or len(str(byte_ev.get('sha256',''))) != 64 or int(byte_ev.get('bytes') or 0)<=0:
            raise SystemExit(f'byte-grounding failed: {eid}')
    ...
    OUT.write_text(json.dumps({'schema':'dore.italian-editorial-real-e2e.v1','status':'ARTIFACTS_READY','cases':rows},ensure_ascii=False,indent=2),encoding='utf-8')
    print('DORÉ_ITALIAN_EDITORIAL_REAL_E2E=ARTIFACTS_READY')
    print(OUT)
    return 0
