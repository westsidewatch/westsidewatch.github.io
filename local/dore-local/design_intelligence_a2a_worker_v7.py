...
def _repair_candidate(ollama,payload,candidate,domains,attempt):
    ...
    user=json.dumps({
      ...
      'current_patch':candidate.get('patch'),
      'repair_attempt':attempt,
    },ensure_ascii=False)
    raw=legacy._json_object(ollama([{'role':'system','content':system},{'role':'user','content':user}]))
    patch=raw.get('repair_patch') or {}
    validation=repair.validate_repair_ops(domains,patch)
    if not validation.get('ok'):
        raise ValueError('repair_patch_invalid:'+str(validation.get('reason')))
    repaired=legacy.sandbox.materialize(candidate['snapshot'],patch,candidate['candidate_id'])
    repaired['original_patch']=candidate.get('patch')
    repaired['repair_patch']=patch
    repaired['risk_domains']=[str(x) for x in (raw.get('risk_domains') or [])]
    repaired['repair_history']=(candidate.get('repair_history') or [])+[{
        'attempt':attempt,'domains':list(domains),'validation':validation,
    }]
...
def _repair_blocked(ollama,payload,candidates,enforcement_result):
    ...
    for attempt in range(1,repair.MAX_REPAIR_ATTEMPTS+1):
        ...
        for item in blocked:
            ...
            domains=[str(x) for x in (item.get('repeated_failure_domains') or [])]
            source=by_id.get(cid)
            if source is None:
                raise RuntimeError('repair_candidate_missing:'+cid)
            repaired,event=_repair_candidate(ollama,payload,source,domains,attempt)
            trace.append({'candidate_id':cid,**event})
            if repaired is None:
                return current,enforcement_result,trace,False
            by_id[cid]=repaired
            current=[by_id[str(c.get('candidate_id'))] for c in current]
            changed=True
        if not changed:
            break
        enforcement_result=enforcement.enforce(current,v6._guardrails(payload))
        if enforcement_result.get('all_admitted'):
            return current,enforcement_result,trace,True
    return current,enforcement_result,trace,bool(enforcement_result.get('all_admitted'))


def _model(payload):
    from dore_local import ollama
    base=payload['base_snapshot']; nodes=legacy.sandbox.geometry_evidence(base)
    guardrails=v6._guardrails(payload)
    variants,candidates=v6._generate(ollama,payload,base,nodes,0)
    enforcement_result=enforcement.enforce(candidates,guardrails)
    repair_trace=[]
    repaired=False
    if not enforcement_result.get('all_admitted'):
        candidates,enforcement_result,repair_trace,repaired=_repair_blocked(ollama,payload,candidates,enforcement_result)
    regeneration_count=0
    while not enforcement_result.get('all_admitted') and regeneration_count<enforcement.MAX_REGENERATIONS:
        regeneration_count+=1
        variants,candidates=v6._generate(ollama,payload,base,nodes,regeneration_count)
        enforcement_result=enforcement.enforce(candidates,guardrails)
        if not enforcement_result.get('all_admitted'):
            candidates,enforcement_result,extra,was_repaired=_repair_blocked(ollama,payload,candidates,enforcement_result)
            repair_trace.extend(extra); repaired=repaired or was_repaired
    if not enforcement_result.get('all_admitted'):
        raise RuntimeError('repair_and_regeneration_exhausted')
    ...
    loser_failures=legacy._consensus_failures(votes) if consensus else []
    ...
    return {
      'variants':variants,'candidates':candidates,'critic':critic,'provider':'dore-local',
      'model':os.environ.get('DORE_MODEL') or os.environ.get('OLLAMA_MODEL') or 'local-default',
      'rejection_enforcement':enforcement_result,
      'repair_loop':{'policy':'minimal-domain-repair-v1','attempts':repair_trace,'repair_used':bool(repair_trace),'repair_succeeded':repaired,'regeneration_count':regeneration_count},
    }
