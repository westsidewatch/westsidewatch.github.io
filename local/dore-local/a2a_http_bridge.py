...
REGISTRY=build_registry();WEB_ORIGINS={'https://chatgpt.com','https://chat.openai.com'}
def allowed_origin(origin):
 return origin in WEB_ORIGINS or origin.startswith('moz-extension://') or origin.startswith('chrome-extension://')
class H(BaseHTTPRequestHandler):
 def _origin(self):return self.headers.get('Origin','')
 def _cors(self):
  origin=self._origin()
  if allowed_origin(origin):self.send_header('Access-Control-Allow-Origin',origin);self.send_header('Vary','Origin');self.send_header('Access-Control-Allow-Headers','Content-Type');self.send_header('Access-Control-Allow-Methods','GET,POST,OPTIONS')
 ...
 def do_OPTIONS(self):
  if not allowed_origin(self._origin()):return self._out(403,{'ok':False,'error':'origin_not_allowed'})
 ...
 def do_POST(self):
  ...
  if self._origin() and not allowed_origin(self._origin()):return self._out(403,{'ok':False,'error':'origin_not_allowed'})
  try:
