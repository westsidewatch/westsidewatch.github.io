...
for term in ["浮現", "Emergence", "Doré Emergence"]:
    result = gate.admit(term)
    assert result["state"] == "admitted", result
    assert result["entity"] == "capability:emergence", result
...
assert gate.AUTHORITY_RANK["github-current-canon"] > gate.AUTHORITY_RANK["accepted-recent-decision"] > gate.AUTHORITY_RANK["durable-history"] > gate.AUTHORITY_RANK["memory-summary"] > gate.AUTHORITY_RANK["model-inference"]
