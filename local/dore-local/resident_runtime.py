...
VERSION='dore.resident-runtime.v0.4'
...
RUNTIME=HOME/'runtime';LEARNING=HOME/'coordination'/'learning';RESEARCH=HOME/'coordination'/'research'
DRIVER=ROOT/'local'/'dore-local'/'autonomous_driver.py';RESEARCH_EXECUTOR=ROOT/'local'/'dore-local'/'research_executor.py';SELF=ROOT/'local'/'dore-local'/'resident_runtime.py'
STATE=RUNTIME/'state.json';EVENTS=RUNTIME/'events.jsonl';HEARTBEAT=RUNTIME/'heartbeat.json';LOCK=RUNTIME/'runtime.lock'
TELEMETRY_REPO=RUNTIME/'telemetry-repo';TELEMETRY_BRANCH=os.environ.get('DORE_RUNTIME_TELEMETRY_BRANCH','dore-runtime-telemetry')
...
PARENT_ID=os.environ.get('DORE_RUNTIME_PARENT_ID','new-westside-storybook-real-loop-2');PARENT_GOAL=os.environ.get('DORE_RUNTIME_PARENT_GOAL','New Westside visual construction')
...
def research_id_for(learning):
    ...
    return f'research-{PARENT_ID}-{fingerprint(fp)[:16]}'
def transition(job_path,job,state,**extra):
    history=list(job.get('history') or []);history.append({'at':now(),'state':state,**{k:v for k,v in extra.items() if k in {'reason','evidence'}}})
    job={**job,'state':state,'updated_at':now(),'history':history,**extra};atomic_json(job_path,job);append_event(state,research_id=job.get('research_id'),parent_goal=PARENT_GOAL);return job
...
def run_research(job_path):
    ...
    cp=subprocess.run([sys.executable,str(RESEARCH_EXECUTOR),str(job_path)],cwd=str(ROOT),text=True,capture_output=True,timeout=600)
    parsed=None
...
def requeue_after_rejected(job_path,job,result):
    diag={'returncode':result.get('returncode'),'stdout_tail':(result.get('stdout') or '')[-2500:],'stderr_tail':(result.get('stderr') or '')[-2500:],'parsed_result':result.get('result')}
    ex=list(job.get('experiments') or []);ex.append({'at':now(),'status':'REJECTED','diagnostic':diag})
    job={**job,'iteration':int(job.get('iteration') or 1)+1,'experiments':ex,'knowledge_artifact':None,'question':str(job.get('question') or '')+' Previous experiment was rejected. Research this new evidence before another execution: '+json.dumps(diag,ensure_ascii=False)[-3000:]}
    return transition(job_path,job,'RESEARCH_QUEUED',reason='experiment_rejected_new_evidence')
def finish_verified(job_path,job,result):
    job=transition(job_path,job,'VERIFIED',evidence='parent-specific experiment passed')
    parsed=result.get('result') if isinstance(result.get('result'),dict) else {};promoted=parsed.get('promoted_skill') or parsed.get('promotion') or parsed.get('skill')
    job=transition(job_path,job,'PROMOTED',promotion=promoted or 'verified capability/evidence retained')
    return transition(job_path,job,'RESUME_PARENT',resumed=True)
def telemetry_snapshot():
    ...
    return {'schema':'dore.runtime.telemetry.v0.3','published_at':now(),'runtime':VERSION,'host_role':'dore-local-mac','parent_goal':PARENT_GOAL,'parent_message_id':PARENT_ID,'heartbeat':heartbeat,'state':state,'research_jobs':jobs,'events':tail_events(40)}
...
def publish_telemetry(force=False):
    ...
    repo=ensure_telemetry_repo();run(['git','fetch','origin',TELEMETRY_BRANCH],cwd=repo,timeout=90);exists=run(['git','show-ref','--verify',f'refs/remotes/origin/{TELEMETRY_BRANCH}'],cwd=repo,timeout=30).returncode==0
...
def maybe_self_update():
    ...
    changed=[];targets=[('local/dore-local/resident_runtime.py',SELF),('local/dore-local/autonomous_driver.py',DRIVER),('local/dore-local/research_executor.py',RESEARCH_EXECUTOR),('local/dore-local/autonomous_capability_loop.py',ROOT/'local'/'dore-local'/'autonomous_capability_loop.py'),('dore-design/knowledge-lab/resources/source-catalog.json',ROOT/'dore-design'/'knowledge-lab'/'resources'/'source-catalog.json')]
...
def tick():
    ...
    if learning and learning.get('state')=='RESEARCH_REQUIRED':
        ...
        if job.get('state') in {'RESEARCH_QUEUED','RESEARCHING','RESEARCH_BLOCKED'}:
            append_event('RESEARCHING',research_id=job.get('research_id'),parent_goal=PARENT_GOAL);rr=run_research(job_path);job=read_json(job_path,job) or job
            state={**state,'last_research_diagnostic':rr,'research_id':job.get('research_id'),'research_job':str(job_path),'last_event':job.get('state'),'driver_passed':False};atomic_json(STATE,state)
            atomic_json(HEARTBEAT,{'runtime':VERSION,'at':now(),'state':job.get('state'),'parent_goal':PARENT_GOAL,'research_id':job.get('research_id'),'next_tick_seconds':INTERVAL});publish_telemetry(force=True)
            if job.get('state')!='KNOWLEDGE_RETURNED':return
        if job.get('state') in {'PEER_RESEARCH_QUEUED'}:
            atomic_json(HEARTBEAT,{'runtime':VERSION,'at':now(),'state':'PEER_RESEARCH_QUEUED','parent_goal':PARENT_GOAL,'research_id':job.get('research_id'),'next_tick_seconds':INTERVAL});return
        if job.get('state') in {'KNOWLEDGE_RETURNED','VERIFIED','PROMOTED'}:
            job=transition(job_path,job,'EXPERIMENTING',evidence='knowledge artifact available');reason='KNOWLEDGE_RETURNED_EXPERIMENT'
    ...
    append_event('CONTINUE',reason=reason,parent_goal=PARENT_GOAL,learning=str(lp) if lp else None);before=time.time();result=run_driver(reason,lp,learning,job_path,job);progressed=bool(result.get('ok')) or fingerprint(result)!=state.get('last_result_fingerprint')
    ...
    atomic_json(STATE,new);atomic_json(HEARTBEAT,{'runtime':VERSION,'at':now(),'state':'PASS' if result.get('ok') else ('RESEARCH_QUEUED' if job_path else 'RUNNING_WITH_FAILURE_EVIDENCE'),'parent_goal':PARENT_GOAL,'last_event':new.get('last_event'),'driver_ok':bool(result.get('ok')),'next_tick_seconds':INTERVAL})
    append_event('DRIVER_RESULT',reason=reason,ok=bool(result.get('ok')),returncode=result.get('returncode'),information_gain=progressed,result=(result.get('result') or {}).get('error') if isinstance(result.get('result'),dict) else None);publish_telemetry(force=True)
def main():
    RUNTIME.mkdir(parents=True,exist_ok=True)
