...
def main():
 ...
 pyfiles=['resident_runtime.py','goal_queue.py','autonomous_driver.py','autonomous_capability_loop.py','research_executor.py','peer_research_bridge.py','failure_memory.py','shared_learning.py','a2a_adapter.py','coordination_mailbox.py','coordination_worker.py']
 for name in pyfiles:
 ...
 runtime=source('local/dore-local/resident_runtime.py')
 for token in ['RESEARCH_QUEUED','RESEARCHING','PEER_RESEARCH_QUEUED','KNOWLEDGE_RETURNED','EXPERIMENTING','VERIFIED','REJECTED','PROMOTED','RESUME_PARENT','research_executor.py','peer_research_bridge.py','failure_memory','shared_learning','a2a_adapter','goal_queue','project-state.json','SELF_UPDATED','last_driver_diagnostic','synthetic_gap','+refs/heads/main:refs/remotes/origin/main']:mark(checks,'runtime owns '+token,token in runtime)
 ...
 with tempfile.TemporaryDirectory(prefix='dore-loop-accept-') as td:
  ...
  # Goal queue executable smoke: enqueue -> active -> PASS.
  qenv={**env};code="import sys,json;sys.path.insert(0,'local/dore-local');import goal_queue as q;q.enqueue('accept-goal','accept');a=q.current();p=q.set_status('accept-goal','PASS');print(json.dumps({'active':a.get('status'),'pass':p.get('status')}))";cp=subprocess.run([sys.executable,'-c',code],cwd=str(ROOT),env=qenv,text=True,capture_output=True,timeout=30)
 ...
 passed=all(x['pass'] for x in checks);print(json.dumps({'ok':passed,'schema':'dore.loop-contract-acceptance.v4','root':str(ROOT),'checks':checks,'passed':sum(1 for x in checks if x['pass']),'total':len(checks)},ensure_ascii=False));return 0 if passed else 1
