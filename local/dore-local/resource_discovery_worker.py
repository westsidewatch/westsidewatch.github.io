...
def ensure_schema(c):
 ...
 '''); c.commit()
def headers():
 h={'Accept':'application/vnd.github+json','User-Agent':'Dore-Resource-Discovery/1.0'}
...
def upsert(c,family,item):
 ...
 lic=item.get('license') or {}; evidence={'github_id':item.get('id'),'full_name':item.get('full_name'),'default_branch':item.get('default_branch'),'fork':item.get('fork'),'open_issues_count':item.get('open_issues_count'),'topics':item.get('topics') or [],'language':item.get('language'),'homepage':item.get('homepage')}
 c.execute('''INSERT INTO dore_resource_candidates(id,canonical_url,name,source,capability_family,description,discovered_at,last_seen_at,source_updated_at,stars,license_spdx,archived,status,evidence_json)
 ...
 (rid(url),url,str(item.get('full_name') or item.get('name') or url),'github',family,item.get('description'),now(),now(),item.get('updated_at'),int(item.get('stargazers_count') or 0),lic.get('spdx_id'),1 if item.get('archived') else 0,'candidate',json.dumps(evidence,ensure_ascii=False)))
 return not existed
def main():
 ...
 try:
  ...
  status='ok' if not errors else ('partial' if total else 'error'); finished=now(); evidence={'queries':[{'family':f,'query':q} for f,q in QUERIES],'errors':errors,'policy':'candidate-only; no auto-install; revalidate-before-use'}
  ...
  state={'ok':status!='error','run_id':run,'checked_at':finished,'candidate_count':total,'new_count':new,'errors':errors,'policy':'Discover continuously. Revalidate before use. Reuse when better. Build only when necessary.'};STATE.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding='utf-8');emit('complete',**state);return 0 if status!='error' else 1
