...
from editorial_visual_observation_capability import (
    HistoricalImageEvidence,
    observe_historical_editorial_image,
)


def evidence_to_historical_image(item: Mapping[str, Any]) -> HistoricalImageEvidence:
    return HistoricalImageEvidence(
        evidence_id=str(item.get("id", "")),
        image_uri=str(item.get("image", "")),
        source_uri=str(item.get("source", "")),
        publication=str(item.get("family", "")),
        era=str(item.get("era", "")),
    )


def observe_atlas_evidence(
    ...
    *,
    ...
    """Observe one exact Atlas image without feeding existing prose/fingerprint to vision."""
    evidence = evidence_to_historical_image(item)
    result = observe_historical_editorial_image(
        evidence,
        provider,
    ...
    result["atlasBinding"] = {
        ...
        "era": item.get("era"),
        "historicalAuthority": item.get("authority"),
    }
    result["inputIsolation"] = {
        "whatToNoticeWasNotObserverInput": True,
        "grammarTokensWereNotObserverInput": True,
        "handwrittenFingerprintWasNotObserverInput": True,
    }
    return result
