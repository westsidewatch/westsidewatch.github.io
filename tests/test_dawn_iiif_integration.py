...
class DawnIiifIntegrationTests(unittest.TestCase):
    ...
    def test_registry_records_real_iiif_runtime_as_mounted(self):
        registry = SurfaceMountRegistry()
        mount = registry.get('iiif-visual-surface')
        self.assertIsNotNone(mount)
        self.assertEqual(mount.state, 'mounted')
        self.assertEqual(mount.implementation, 'static/js/dawn-visual-viewer.js')
        self.assertTrue(registry.executable('iiif-visual-surface'))
