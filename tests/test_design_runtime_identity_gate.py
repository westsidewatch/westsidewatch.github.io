...
class RuntimeIdentityGateTest(unittest.TestCase):
    ...
    def test_rejects_design1(self):
        bad = dict(GOOD, version="1.0")
        with patch.object(resident_design, "_json_get", return_value=bad):
            with self.assertRaisesRegex(RuntimeError, "identity mismatch"):
                resident_design._runtime_identity()

    def test_rejects_wrong_entrypoint(self):
        bad = dict(GOOD, entrypoint="dore-design/app_design.py")
        with patch.object(resident_design, "_json_get", return_value=bad):
    ...
    def test_precondition_filters_unknown_fields(self):
