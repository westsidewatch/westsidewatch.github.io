...
class CompanionNativeContractTest(unittest.TestCase):
 ...
 def test_site_capability_bridge_is_bounded_to_declared_site_capabilities(self):
  ...
  for forbidden in ('QMD','Concord','SWORD','OpenAI'):self.assertNotIn(forbidden,bridge)
 ...
 def test_self_maintenance_is_bounded_and_reversible(self):
  ...
  self.assertIn('"self_maintenance_action"',host);self.assertIn('_execute(name,cap,args)',host);self.assertIn('system.self-maintain',action)
