...
# Multiwrite site capabilities are explicitly declared and bounded; publication admission is not an open-ended bridge.
...
class CompanionNativeContractTest(unittest.TestCase):
 ...
 def test_site_capability_bridge_is_bounded_to_declared_site_capabilities(self):
  ...
  self.assertIn('SITE_CAPABILITY_ALLOWLIST=new Set(["context.fuzzy-search","publishing.book-intelligence","image.generate","dawn.library.publish"])',bg);self.assertIn('message.type==="dore.site-capability"',bg);self.assertIn('caller_product:',bg)
  self.assertIn("const FUZZY_CAPABILITY='context.fuzzy-search'",bridge);self.assertIn("const BOOK_CAPABILITY='publishing.book-intelligence'",bridge);self.assertIn("const IMAGE_CAPABILITY='image.generate'",bridge);self.assertIn("const DAWN_CAPABILITY='dawn.library.publish'",bridge)
  ...
  self.assertIn("'dore:cover-generate'",bridge);self.assertIn("'dore:cover-generate-result'",bridge);self.assertIn("await send(IMAGE_CAPABILITY,args,'multiwrite')",bridge)
  self.assertIn("'dore:dawn-publish'",bridge);self.assertIn("'dore:dawn-publish-result'",bridge);self.assertIn("await send(DAWN_CAPABILITY,args,'multiwrite')",bridge)
