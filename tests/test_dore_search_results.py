...
def test_merge_results_deduplicates_and_preserves_provenance():
    qmd = {
        "ok": True,
    ...
    assert results[0]["title"] == "嗎哪與曠野"
    assert results[0]["score"] == 0.91
    assert {p["kind"] for p in results[0]["provenance"]} == {"document", "memory"}
    assert "qmd" not in str(results[0]).lower()
    assert "longmemory" not in str(results[0]).lower()


def test_passive_fuzzy_search_stays_light_and_returns_clean_results():
    calls = {"memory": 0}

    def qmd_search(text, *, semantic, deep, limit):
        assert text == "這句讓我想到曠野裡的嗎哪"
    ...
    assert result["results"][0]["title"] == "嗎哪與曠野"
    assert result["results"][0]["provenance"][0]["kind"] == "document"


def test_explicit_search_may_add_current_memory_and_merge_duplicate():
    calls = {"memory": 0}
    ...
    assert {p["kind"] for p in result["results"][0]["provenance"]} == {"document", "memory"}
