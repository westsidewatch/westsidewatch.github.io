...
const result=await page.evaluate(async()=>{
  ...
  const envelope=(overrides={})=>{
    const base={
      ok:true,status:'completed',schema:'dore.source-capability-envelope.v1',sourcePointer:'https://example.org/source',
      authority:{sourceIsAuthority:true,identityClaimOnly:true,canonicalIdentityAuthority:false,envelopeAuthority:false},
      access:{modes:['static-http'],preferred:'static-http'},media:['text'],operations:['cite','read','extract'],
      ...
      dispatch:{ok:true,status:'ready',schema:'dore.source-dispatch.v1',mode:'static-http',routeClass:'static',requiresRuntime:false,materializationReady:true,providerSpecificRouting:false,authority:false,persistence:'request-scoped-none'},
      editorialBoundary:{canonRequiredFor:['publish','material-transform','rehost'],sourceContentMayBeRewrittenSilently:false},
      surfaceHints:['dawn','multiwrite'],provenance:{probeSchema:'dore.source-probe.v0',probeStatus:'completed'},persistence:'request-scoped-none'
    };
  ...
  const deferred=admitSourceUses([{id:'runtime-source',use:'read',envelope:envelope({access:{modes:['browser-runtime'],preferred:'browser-runtime'},runtimeBoundary:{required:true,mode:'browser-runtime'},dispatch:{ok:true,status:'runtime-required',schema:'dore.source-dispatch.v1',mode:'browser-runtime',routeClass:'runtime',requiresRuntime:true,materializationReady:false,providerSpecificRouting:false,authority:false,persistence:'request-scoped-none'}})}]);
  const missingDispatch=admitSourceUses([{id:'missing-dispatch',use:'read',envelope:envelope({dispatch:null})}]);
  ...
  return{status:mat.status,acceptance:mat.acceptance,epubHead:await head(mat.files.epub.blob,2),pdfHead:String.fromCharCode(...await head(mat.files.pdf.blob,5)),sizes:{epub:mat.files.epub.blob.size,pdf:mat.files.pdf.blob.size,web:mat.files.web.blob.size},cover:Boolean(mat.files.cover),wiring:{epub:Boolean(document.querySelector('[data-formal-export="epub"]')),web:Boolean(document.querySelector('[data-formal-export="web"]')),pdfHook:document.querySelector('[data-export="pdf"]')?.dataset.legacyPdf==='1'},capability:{admitted:admitted.status,rightsBlocked:rightsBlocked.status,rightsReason:rightsBlocked.rows[0]?.reason,rightsAdmitted:rightsAdmitted.status,deferred:deferred.status,deferredReason:deferred.rows[0]?.reason,missingDispatch:missingDispatch.status,missingDispatchReason:missingDispatch.rows[0]?.reason,forbiddenAuthority:forbiddenAuthority.status,autopilotState:blockedAutopilot.state,autopilotArtifacts:Boolean(blockedAutopilot.artifacts),privateAdmissionStored:Boolean(blockedBook.internalProvenance?.sourceCapabilityAdmission),publicAdmissionLeaked:Object.prototype.hasOwnProperty.call(publicProjection,'internalProvenance')}};
});
await browser.close();
...
assert.ok(result.sizes.epub>200);assert.ok(result.sizes.pdf>1000);assert.ok(result.sizes.web>100);assert.equal(result.cover,true);
assert.deepEqual(result.wiring,{epub:true,web:true,pdfHook:true});
assert.deepEqual(result.capability,{admitted:'admitted',rightsBlocked:'blocked',rightsReason:'rights-admission-required:rehost',rightsAdmitted:'admitted',deferred:'deferred',deferredReason:'runtime-capability-required',missingDispatch:'blocked',missingDispatchReason:'source-dispatch-missing-or-invalid',forbiddenAuthority:'blocked',autopilotState:'blocked',autopilotArtifacts:false,privateAdmissionStored:true,publicAdmissionLeaked:false});
