"""Living Water Candidate 04 — weighted Doré currents with verifiable native light engines."""
import html
...
def page():
    return {'id':PAGE_ID,'name':'Living Water · Candidate 04','canvas':{'w':1440,'h':960},'nodes':[], 'design_experiment':{'schema':'dore.design-experiment.v1','track':'living-water-homepage','candidate':'04','status':'design-candidate','source':'user-direction-2026-09-09','principles':['inherit-c01-after-page2','four-horizontal-currents','journal-four-movements-internal-only','never-show-W-labels','8:5-weighted-rhythm','editorial-item-model','per-item-dore-original','C-god-rays-classic-webgl','D-react-bits-lightrays-classic-webgl','no-esm-runtime-dependency','runtime-fps-probe','light-over-content','no-third-party-demo-iframe','first-layer-only']}}
...
s.querySelectorAll('[data-light]').forEach(function(btn){btn.onclick=function(){mode=btn.dataset.light;C.classList.toggle('off',mode!=='c');D.classList.toggle('off',mode!=='d');s.querySelectorAll('[data-light]').forEach(function(x){x.classList.toggle('on',x===btn)});status.textContent=mode==='c'?'C STARTING':mode==='d'?'D STARTING':'LIGHT OFF'}});
function build(canvas,host,fragment,label){
  try{
    ...
    var prog=gl.createProgram();gl.attachShader(prog,shader(gl.VERTEX_SHADER,vs));gl.attachShader(prog,shader(gl.FRAGMENT_SHADER,fragment));gl.linkProgram(prog);if(!gl.getProgramParameter(prog,gl.LINK_STATUS))throw new Error(gl.getProgramInfoLog(prog));gl.useProgram(prog);
    ...
    var U={time:gl.getUniformLocation(prog,'iTime'),res:gl.getUniformLocation(prog,'iResolution'),mouse:gl.getUniformLocation(prog,'mousePos')},mouse=[.5,.5],smooth=[.5,.5],frames=0,last=performance.now();
    function resize(){var r=host.getBoundingClientRect(),dpr=Math.min(devicePixelRatio||1,1.5),w=Math.max(2,Math.floor(r.width*dpr)),h=Math.max(2,Math.floor(r.height*dpr));if(canvas.width!==w||canvas.height!==h){canvas.width=w;canvas.height=h;gl.viewport(0,0,w,h)}}
    addEventListener('resize',resize);addEventListener('mousemove',function(e){var r=host.getBoundingClientRect();if(r.width&&r.height)mouse=[(e.clientX-r.left)/r.width,(e.clientY-r.top)/r.height]},{passive:true});resize();
    return function(now){var active=(label==='C'&&mode==='c')||(label==='D'&&mode==='d');if(!active)return;resize();smooth[0]=smooth[0]*.92+mouse[0]*.08;smooth[1]=smooth[1]*.92+mouse[1]*.08;gl.useProgram(prog);gl.uniform1f(U.time,now*.001);gl.uniform2f(U.res,canvas.width,canvas.height);gl.uniform2f(U.mouse,smooth[0],smooth[1]);gl.drawArrays(gl.TRIANGLES,0,3);frames++;if(now-last>600){status.textContent=label+' RUNNING · '+Math.round(frames*1000/(now-last))+' FPS';frames=0;last=now}}
  }catch(err){console.warn(label+' light runtime failed',err);return function(){if((label==='C'&&mode==='c')||(label==='D'&&mode==='d'))status.textContent=label+' ERROR'}}
}
var cFrag='precision highp float;uniform float iTime;uniform vec2 iResolution;uniform vec2 mousePos;float beam(vec2 uv,float x,float width,float sway){float center=x+sin(iTime*sway+x*8.0)*0.08;float dx=abs(uv.x-center);float shaft=smoothstep(width,0.0,dx);float fall=smoothstep(1.10,-0.05,uv.y);return shaft*fall;}void main(){vec2 uv=gl_FragCoord.xy/iResolution.xy;uv.y=1.0-uv.y;float drift=(mousePos.x-.5)*.10;float b=0.0;b+=beam(uv,.18+drift,.095,.31)*.55;b+=beam(uv,.38+drift*.7,.055,.43)*.82;b+=beam(uv,.57+drift*.4,.12,.22)*.38;b+=beam(uv,.74+drift*.9,.065,.37)*.76;b+=beam(uv,.88+drift*.5,.035,.53)*.52;float haze=.12+.10*sin(iTime*.35+uv.x*7.0);float pulse=.82+.18*sin(iTime*.72);b=(b+haze*smoothstep(.95,.15,uv.y))*pulse;vec3 col=vec3(1.0,.88,.52)*b;float a=clamp(b*.58,0.0,.58);gl_FragColor=vec4(col,a);}';
var dFrag='precision highp float;uniform float iTime;uniform vec2 iResolution;uniform vec2 mousePos;float strength(vec2 s,vec2 d,vec2 c,float a,float b,float sp){vec2 v=c-s;vec2 n=normalize(v);float ca=dot(n,d);float dc=ca+.24*sin(iTime*2.0+length(v)*.01)*.2;float sf=pow(max(dc,0.0),1.0/.48);float dist=length(v),md=iResolution.x*2.1,lf=clamp((md-dist)/md,0.0,1.0);float pulse=.8+.2*sin(iTime*sp*3.0);float base=clamp((.45+.15*sin(dc*a+iTime*sp))+(.3+.2*cos(-dc*b+iTime*sp)),0.0,1.0);return base*lf*sf*pulse;}void main(){vec2 c=vec2(gl_FragCoord.x,iResolution.y-gl_FragCoord.y);vec2 s=vec2(iResolution.x*.50,-iResolution.y*.20);vec2 md=normalize(mousePos*iResolution.xy-s),fd=normalize(mix(vec2(0.0,1.0),md,.10));float e=strength(s,fd,c,36.2214,21.11349,1.875)*.5+strength(s,fd,c,22.3991,18.0234,1.375)*.4;float br=1.0-c.y/iResolution.y;vec3 col=e*vec3(.28+br*.72,.34+br*.66,.18+br*.52)*vec3(.88,.78,.42);gl_FragColor=vec4(col,clamp(e*.72,0.0,.68));}';
var drawC=build(document.getElementById('godRaysCanvas'),C,cFrag,'C'),drawD=build(document.getElementById('lightRaysCanvas'),D,dFrag,'D');
(function loop(now){requestAnimationFrame(loop);drawC(now);drawD(now)})(0);
})();
</script>'''
