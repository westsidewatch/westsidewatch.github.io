...
SHARED_CSS='.dore-bound{white-space:pre-line}'
EDITOR_CSS='''body[data-dore-editor="true"]{padding-top:44px}.dore-editbar{position:fixed;left:0;right:0;top:0;height:44px;z-index:10000;background:#171717;color:#f2eee4;display:flex;align-items:center;gap:8px;padding:0 12px;font:11px ui-monospace,SFMono-Regular,Menlo,monospace;box-shadow:0 1px 0 #000}.dore-editbar strong{letter-spacing:.08em}.dore-editbar a{color:#f2eee4;background:#303030;border:1px solid #555;padding:6px 9px;text-decoration:none;font:inherit}.dore-editbar .state{margin-left:auto;color:#d0bd78}body[data-dore-editor="true"] .dore-bound{cursor:text;outline:1px dashed transparent;outline-offset:4px}body[data-dore-editor="true"] .dore-bound:hover{outline-color:rgba(179,154,71,.7)}body[data-dore-editor="true"] .dore-bound:focus{outline:2px solid #d0bd78;background:rgba(208,189,120,.08)}'''
CLIENT_JS=r'''<script>(()=>{const edit=document.body.dataset.doreEditor==='true';const esc=s=>String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));let workspace=null,home=null;function node(id){return home?.nodes?.find(n=>n.id===id)}function paint(el,value,id){value=String(value??'');if(id==='home-title'){const lines=value.split(/\n+/).filter(Boolean);el.innerHTML=lines.map((x,i)=>i===lines.length-1?'<em>'+esc(x)+'</em>':esc(x)).join('<br>')}else el.textContent=value}async function load(){const r=await fetch('/api/workspace',{cache:'no-store'});workspace=await r.json();home=workspace.pages.find(p=>p.id==='homepage')||workspace.pages[0];document.querySelectorAll('[data-node-id]').forEach(el=>{const n=node(el.dataset.nodeId),field=el.dataset.field||'text';if(n&&n[field]!==undefined)paint(el,n[field],n.id);if(edit){el.contentEditable='true';el.spellcheck=false}});const st=document.getElementById('dore-state');if(st)st.textContent='workspace r'+workspace.revision+' · exact #262 layout'}async function save(el){const id=el.dataset.nodeId,field=el.dataset.field||'text';let value=el.innerText.replace(/\n{3,}/g,'\n\n').trim();const r=await fetch('/api/workspace',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({op:'set_node',page_id:'homepage',id,patch:{[field]:value}})});if(!r.ok){document.getElementById('dore-state').textContent='SAVE FAILED';return}workspace=await r.json();home=workspace.pages.find(p=>p.id==='homepage')||workspace.pages[0];paint(el,value,id);document.getElementById('dore-state').textContent='saved · r'+workspace.revision}if(edit){document.addEventListener('blur',e=>{const el=e.target.closest?.('[data-node-id]');if(el)save(el)},true);document.addEventListener('click',e=>{if(e.target.closest('.portal,.gate-mark')&&!e.target.closest('.dore-editbar'))e.preventDefault()},true)}load().catch(e=>{const st=document.getElementById('dore-state');if(st)st.textContent='workspace load failed';console.error(e)})})();</script>'''
def render(edit=False):
 html=TEMPLATE.read_text(encoding='utf-8').replace('/images/westside-watch-masthead-landscape.svg','/asset/masthead.svg').replace('/images/westside-watch-morning-star.svg','/asset/morning-star.svg')
 for old,new in BINDINGS:html=html.replace(old,new)
 html=html.replace('</style>',SHARED_CSS+(EDITOR_CSS if edit else '')+'</style>',1)
 if edit:
  html=html.replace('<body>','<body data-dore-editor="true">',1);bar='<div class="dore-editbar"><strong>DORÉ DESIGN 1.5 · WYSIWYG</strong><a href="/">Preview</a><a href="/structure-editor">Structure</a><a href="/journal/">Journal Mirror</a><span id="dore-state" class="state">loading workspace…</span></div>';html=html.replace('<main class="shell"',bar+'<main class="shell"',1)
 return html.replace('</body>',CLIENT_JS+'</body>',1)
