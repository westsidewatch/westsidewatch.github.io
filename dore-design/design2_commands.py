...
def align_nodes(w,pid,ids,edge):
    ...
    page=_page(w,pid);ns=[_node(page,i) for i in ids];bs=[_box(n) for n in ns]
    L=min(x for x,y,a,b in bs);R=max(x+a for x,y,a,b in bs);T=min(y for x,y,a,b in bs);B=max(y+b for x,y,a,b in bs);ps=[]
    for n,(x,y,a,b) in zip(ns,bs):
        if edge=='left':p={'x':L}
        elif edge=='center':p={'x':(L+R-a)/2}
        elif edge=='right':p={'x':R-a}
        elif edge=='top':p={'y':T}
        elif edge=='middle':p={'y':(T+B-b)/2}
        else:p={'y':B-b}
        ps.append({'id':n['id'],'patch':p})
...
def distribute(w,pid,ids,axis):
    ...
    if axis=='horizontal':
        start=boxes[0][0];end=boxes[-1][0]+boxes[-1][2];total=sum(b[2] for b in boxes);gap=(end-start-total)/(len(ns)-1);cursor=start
        for n,b in zip(ns,boxes):ps.append({'id':n['id'],'patch':{'x':cursor}});cursor+=b[2]+gap
    else:
        start=boxes[0][1];end=boxes[-1][1]+boxes[-1][3];total=sum(b[3] for b in boxes);gap=(end-start-total)/(len(ns)-1);cursor=start
