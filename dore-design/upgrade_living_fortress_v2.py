#!/usr/bin/env python3
"""Restore the approved Watch for the Dawn front door into the durable workspace.

This keeps Doré Design 1.4's single-source architecture: editor and preview both
read the same structured homepage. Only the homepage is replaced; all secondary
pages and workspace history remain preserved.
"""
...
SURFACE='homepage-watch-for-the-dawn-workspace-v3'
...
def homepage():
    nodes=[
        ...
        text('home-deck','在黑夜仍然守望，在清晨尚未來到以前保存光。\n文章、聖經、教會生活與研究，在同一座城中彼此照亮。',68,500,505,24),
        ...
        block('one-territory','ONE · BIBLE STUDY','路上，祂向我們\n打開聖經。','從一章、一卷書、一條路線開始，把經文、歷史、地圖與串珠重新連起來。',552,825,584,270),
        block('church-territory','LIVING WATER WEST','一座城，\n也是一個家','Worship · Bible Study · Prayer · Life Together',552,1113,282,472),
        block('library-territory','DAWN LIBRARY','被保存的光','Maps · essays · history · research',852,1113,284,226),
        block('join-territory','THE GATE · JOIN','進來，\n與我們一同守望','Sunday Worship · Bible Study · Prayer',852,1357,284,228),
        text('gate-copy-left','WATCH TOGETHER.',64,1678,360,30),
        text('gate-mark','THE GATE',526,1765,150,14),
        text('gate-copy-right','WAIT FOR\nTHE DAWN.',820,1655,316,30),
        rule('gate-left',64,1828,445),
        rule('gate-right',691,1828,445),
        text('watch-number','01',64,1935,250,128,'hero'),
        text('watch-quote','「黑夜已深，\n白晝將近。」',420,1930,620,54,'hero'),
        text('watch-cite','ROMANS 13:12',424,2080,300,14),
        rule('footer-rule',64,2175,1072),
        text('footer','WESTSIDE WATCH · WATCH FOR THE DAWN',64,2210,650,13),
    ]
    return {
        'id':'homepage',
        'name':'Homepage / Watch for the Dawn',
...
def main():
    ...
    if workspace.get('active_surface')==SURFACE and current and any(n.get('id')=='join-territory' for n in current.get('nodes',[])):
        print(json.dumps({'ok':True,'code':'WATCH_FOR_DAWN_ALREADY_CURRENT','revision':workspace.get('revision'),'surface':SURFACE},ensure_ascii=False))
        return
    HIST.mkdir(parents=True,exist_ok=True)
    stamp=datetime.datetime.now().strftime('%Y%m%dT%H%M%S')
    shutil.copy2(WS,HIST/f'westside-watch.before-watch-for-dawn-{stamp}.json')
    ...
    print(json.dumps({
        'ok':True,
        'code':'WATCH_FOR_DAWN_WORKSPACE_PASS',
