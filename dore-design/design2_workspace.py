...
def navigate(base,direction,expected_revision):
 ...
 # Workspace save() is monotonic, so redo cannot be inferred from revision numbers.
 # Maintain a small process-local cursor of logical snapshots while persisted
 # history remains the source of truth for undo targets.
...
def install(base):
 ...
 def mutate(workspace,payload):
  ...
  # A new edit after undo establishes a new branch and invalidates redo.
  state=getattr(base,'_design2_history_state',None)
