...
(()=>{
 ...
 const assemble=async card=>{if(focused!==card)return;setPhase('assemble');if(!buildMosaic(card)){setPhase('focus');return}await animatePieces(true);if(focused!==card||phase==='release')return;await mosaic.master.animate([{opacity:0},{opacity:1}],{duration:180,easing:'ease-out',fill:'forwards'}).finished.catch(()=>{});if(!mosaic||phase==='release')return;mosaic.pieces.forEach(p=>p.style.opacity='0');setPhase('focus')};
 ...
 const release=()=>{if(!focused||phase==='release')return releasePromise||Promise.resolve();setPhase('release');const run=async()=>{if(mosaic){await mosaic.master.animate([{opacity:+getComputedStyle(mosaic.master).opacity||1},{opacity:0}],{duration:150,easing:'ease-in',fill:'forwards'}).finished.catch(()=>{});if(mosaic){mosaic.pieces.forEach(p=>p.style.opacity='1');await animatePieces(false)}}cleanup();focused=null;originRects=null;motion.forEach(m=>{m.offset=0;m.velocity=0;m.target=0});render();setPhase('settled');settleUntil=performance.now()+220;releasePromise=null};releasePromise=run();return releasePromise};
 ...
 const pointOnOriginalCard=(x,y)=>originRects?.some(r=>pointInRect(x,y,r,2));
 const pointerPolicy=e=>{if(!focused||phase==='release')return;const x=e.clientX,y=e.clientY;if(phase==='arrest'){const r=focused.getBoundingClientRect();if(!pointInRect(x,y,r,2))release();return}if(phase==='assemble'||phase==='focus'){const onWindow=pointInRect(x,y,mosaic?.target,0),onSource=pointOnOriginalCard(x,y);if(!onWindow&&!onSource)release()}};
