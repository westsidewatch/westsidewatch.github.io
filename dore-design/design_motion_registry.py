...
.dore-mosaic-layer{position:fixed;z-index:2147483000;pointer-events:none;inset:0}.dore-mosaic-piece{position:fixed;overflow:hidden;margin:0;background:#d7cfbd;will-change:left,top,width,height,opacity}.dore-mosaic-piece img{position:absolute;display:block;max-width:none;object-fit:cover;filter:saturate(.58) contrast(.92);will-change:left,top,width,height}.dore-mosaic-master{position:fixed;overflow:hidden;opacity:0;background:#171713;will-change:opacity}.dore-mosaic-master img{width:100%;height:100%;object-fit:cover;display:block;filter:saturate(.62)}
...
(()=>{
 ...
 let phase='settled',focused=null,originRects=null,mosaic=null,last=performance.now(),settleUntil=last+900,releaseTimer=0;
 ...
 const cleanup=()=>{mosaic?.remove();mosaic=null;cards.forEach(c=>c.style.visibility='')};
 const buildMosaic=card=>{cleanup();const group=groupFor(card);if(group.length!==4)return null;originRects=group.map(c=>c.getBoundingClientRect());const target=masterRect(),layer=document.createElement('div');layer.className='dore-mosaic-layer';const src=card.querySelector(':scope > img')?.src||'';const slots=[{x:0,y:0},{x:.5,y:0},{x:0,y:.5},{x:.5,y:.5}];const pieces=group.map((c,i)=>{const r=originRects[i],p=document.createElement('div');p.className='dore-mosaic-piece';Object.assign(p.style,{left:r.left+'px',top:r.top+'px',width:r.width+'px',height:r.height+'px'});const img=document.createElement('img');img.src=src;const slot=slots[i];Object.assign(img.style,{width:(target.width*2)+'px',height:(target.height*2)+'px',left:(-slot.x*target.width*2)+'px',top:(-slot.y*target.height*2)+'px'});p.appendChild(img);layer.appendChild(p);c.style.visibility='hidden';return p});const master=document.createElement('div');master.className='dore-mosaic-master';Object.assign(master.style,{left:target.left+'px',top:target.top+'px',width:target.width+'px',height:target.height+'px'});master.innerHTML=`<img src="${src}" alt="">`;layer.appendChild(master);document.body.appendChild(layer);mosaic={layer,pieces,master,target,group};return mosaic};
 const animatePieces=(forward)=>{if(!mosaic)return Promise.resolve();const {pieces,target}=mosaic,slots=[{x:0,y:0},{x:.5,y:0},{x:0,y:.5},{x:.5,y:.5}];return Promise.all(pieces.map((p,i)=>{const r=originRects[i],s=slots[i],to=forward?{left:target.left+s.x*target.width,top:target.top+s.y*target.height,width:target.width/2,height:target.height/2}:{left:r.left,top:r.top,width:r.width,height:r.height};return p.animate([{left:p.style.left,top:p.style.top,width:p.style.width,height:p.style.height},{left:to.left+'px',top:to.top+'px',width:to.width+'px',height:to.height+'px'}],{duration:forward?560:520,easing:'cubic-bezier(.65,0,.2,1)',fill:'forwards'}).finished.catch(()=>{})}))};
 const assemble=async card=>{if(focused!==card)return;setPhase('assemble');if(!buildMosaic(card)){setPhase('focus');return}await animatePieces(true);if(focused!==card)return;await mosaic.master.animate([{opacity:0},{opacity:1}],{duration:180,easing:'ease-out',fill:'forwards'}).finished.catch(()=>{});mosaic.pieces.forEach(p=>p.style.opacity='0');setPhase('focus')};
 ...
 const release=async()=>{if(!focused||phase==='release')return;clearTimeout(releaseTimer);setPhase('release');if(mosaic){await mosaic.master.animate([{opacity:1},{opacity:0}],{duration:150,easing:'ease-in',fill:'forwards'}).finished.catch(()=>{});mosaic.pieces.forEach(p=>p.style.opacity='1');await animatePieces(false)}cleanup();focused=null;originRects=null;motion.forEach(m=>{m.offset=0;m.velocity=0;m.target=0});render();setPhase('settled');settleUntil=performance.now()+220};
 cards.forEach(card=>{card.addEventListener('mouseenter',e=>{if(e.doreMosaicReplay)return;e.stopImmediatePropagation();requestFocus(card)},true)});
 document.addEventListener('pointermove',e=>{if(!focused||!['assemble','focus'].includes(phase))return;const r=mosaic?.target;if(!r)return;const margin=36;if(e.clientX<r.left-margin||e.clientX>r.left+r.width+margin||e.clientY<r.top-margin||e.clientY>r.top+r.height+margin)release()},{passive:true});document.documentElement.addEventListener('mouseleave',release,true);window.addEventListener('blur',release);
...
<style id="candidate-01-focus-screen-host">.candidate-focus-screen{position:relative;min-height:100svh;height:100svh;overflow:hidden;background:#eee8da;color:#252525}.candidate-focus-screen iframe{display:block;width:100%;height:100%;border:0;background:#eee8da}</style>
'''
...
def _install_living_current(html):
    """Install reversible four-card mosaic focus over the Living Water currents."""
...
def _candidate_focus_screen(screen_no,labels):
    motion_html=_install_living_current(codrops_site_8x5.render(edit=False));motion_html=motion_html.replace('</head>',_CANDIDATE_FOCUS_EMBED_STYLE+'</head>',1);motion_html=motion_html.replace('</body>',_movement_labels_script(labels)+'</body>',1);srcdoc=html_lib.escape(motion_html,quote=True)
...
def _install_candidate_focus(current,html):
    ...
    html=html.replace('</head>',_CANDIDATE_FOCUS_HOST_STYLE+'</head>',1);anchor='</section><section class="world dark">';a=_candidate_focus_screen(2,('第一樂章 WATCH','第二樂章 WITNESS'));b=_candidate_focus_screen(3,('第三樂章 WALK','第四樂章 WORSHIP'))
    return html.replace(anchor,'</section>'+a+b+'<section class="world dark">',1)
