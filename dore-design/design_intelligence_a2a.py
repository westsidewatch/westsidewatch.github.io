...
def explore(payload):
    ...
    request={
      'schema':'dore.design-intelligence-a2a-request.v4','task_id':task_id,'surface_id':payload.get('surface_id'),
      ...
      'evidence_policy':'real-browser-png-primary-v1',
    }
    ...
    return {
      ...
      'variants':variants,'candidates':candidates,'candidate_evidence':'real-browser-png+executable-sandbox-render+geometry','canonical_workspace_mutated':False,
...
def health():
    ...
    return {'ok':worker.exists() and raster.exists(),'phase':9,'capability_phase':12,'policy':'dore-design-pixel-grounded-loop-v1','worker_available':worker.exists(),'rasterizer_available':raster.exists(),'inference_boundary':'core-a2a-only','design_process_has_model_client':False,'blind_order_reversal':True,'minimum_judges':2,'taste_writeback_requires_consensus':True,'executable_candidate_sandbox':True,'real_browser_raster_required':True,'pixel_evidence_primary':True,'canonical_workspace_mutation_allowed':False,'fixture_mode':os.environ.get('DORE_DESIGN_A2A_FIXTURE')=='1','production_promotion':False}
