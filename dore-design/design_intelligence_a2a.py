...
import design_intelligence_assets as visual_assets
...
def _arsenal_inputs(mount):
    ...
    return {'identity_context':outputs.get('westside.context'),'experience_memory':outputs.get('knowledge.recall')}

def _normalize_arsenal_assets(mount,task_id):
    output=(mount.get('outputs') or {}).get('image.generate')
    if not isinstance(output,dict):return [],None
    try:
        asset=visual_assets.normalize_image_output(output,task_id)
...
def explore(payload):
    ...
    arsenal_mount=capability_mount.execute_loadout(arsenal_plan,payload,repo_root=REPO_ROOT,current_capability='design.intelligence',caller_product=str(payload.get('surface_family') or 'design'))
    ...
    arsenal_assets,asset_error=_normalize_arsenal_assets(arsenal_mount,task_id)
    ...
    request={
      ...
      'arsenal_plan':arsenal_plan,'arsenal_mount':arsenal_mount,'arsenal_inputs':arsenal_inputs,'arsenal_assets':arsenal_assets,
    ...
    asset_refs={str(a.get('asset_ref')) for a in arsenal_assets if a.get('asset_ref')}
    used_refs={str(ref) for c in candidates for ref in (c.get('used_asset_refs') or [])}
    used_verified=sorted(asset_refs & used_refs)
    if arsenal_assets and any(not (set(c.get('used_asset_refs') or []) & asset_refs) for c in candidates):
        raise RuntimeError('design_a2a_verified_visual_material_not_used_by_each_candidate')
    if used_verified:
        arsenal_mount=capability_mount.mark_consumed(arsenal_mount,['image.generate'],'candidate-raster-material')
        if 'image.generate' not in consumed_ids:consumed_ids.append('image.generate')
    ...
    loser_failures=[]
    if consensus:loser_failures=failure_domains.normalize_failures(critic.get('loser_failures'))
    ...
    if consensus and memory_admission:
        ...
        route_after=observed.get('updated');loser_direction=str((by_id.get(loser) or {}).get('direction') or loser)
        for failure in loser_failures:
            rejection=intelligence.record_observed_rejection({**payload,'direction':loser_direction,'reason':failure['reason'],'failure_domain':failure['domain'],'confidence':failure['confidence'],'evidence_refs':evidence+['rejected-candidate:'+loser,'failure-domain:'+failure['domain'],'pixel-basis:'+failure['pixel_basis'],'failure-domain-judge-consensus:2'],'scope':payload.get('scope') or 'local'})
            rejections.append({**failure,**rejection,'rejected_candidate':loser,'direction':loser_direction})
    safe_assets=[visual_assets.public_asset_descriptor(a) for a in arsenal_assets]
    return {
      ...
      'variants':variants,'candidates':candidates,'candidate_evidence':'real-browser-png+failure-domains+executable-sandbox-render+geometry+verified-asset-refs','canonical_workspace_mutated':False,
