...
class H(visual.H):
    ...
    def do_GET(self):
        ...
        if path=='/api/health':
            ...
            return self.out(200,{'ok':True,'service':'dore-design','version':'1.7.2','workspace':'new-westside','source_of_truth':'structured-workspace','layout_source':'approved-front-door-262-locked','preview_mode':'multi-page-shared-workspace','coordination_hardening':'1.0','coordination_status':'/api/coordination/status','editor':'/editor','editor_canvas':'/editor-canvas','preview':'/','preview_edit_entry':True,'structure_editor':'/structure-editor','page_count':len(w.get('pages',[])),'journal':'/journal/','journal_page_id':'journal-vol-00','journal_editable':bool(journal),'journal_mode':'editable-workspace-page','journal_source':'one-time-main-site-import','runtime_mirror':False,'journal_asset_fallback':'package+repo-static'})
        p=design_asset(path)
