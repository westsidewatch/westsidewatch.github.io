...
# Keep the old absolute-position canvas only as a structural maintenance tool.
# It is no longer the homepage editor or homepage preview renderer.
STRUCTURE_HTML=(
    visual.HTML.replace(
...
class H(visual.H):
    ...
    def do_GET(self):
        ...
        if path=='/':
            return self.send_bytes(200,homepage_wysiwyg.render(edit=False).encode('utf-8'),'text/html; charset=utf-8')
        if path=='/editor':
            return self.send_bytes(200,homepage_wysiwyg.render(edit=True).encode('utf-8'),'text/html; charset=utf-8')
        if path=='/structure-editor':
            return self.send_bytes(200,STRUCTURE_HTML.encode('utf-8'),'text/html; charset=utf-8')
        ...
        if path=='/api/mirror/status':
            return self.out(200,{
                ...
                'journal':'/journal/','vol00':'/vol-00/',
                'mode':'source-identical-hugo-mirror','redesign':False
            })
        if path=='/api/preview/status':
            ...
            return self.out(200,{
                'ok':bool(home),
                'mode':'same-template-same-workspace-wysiwyg',
                ...
                'editor':'/editor','preview':'/','structure_editor':'/structure-editor'
            })
        ...
        if path=='/api/health':
            return self.out(200,{
                ...
                'layout_source':'approved-front-door-262',
                'preview_mode':'same-template-same-workspace-wysiwyg',
                'design_direction':'watch-for-the-dawn',
                'editor':'/editor','preview':'/','structure_editor':'/structure-editor',
                'journal_mirror':'/journal/','vol00_mirror':'/vol-00/',
                'journal_mode':'source-identical-hugo-mirror',
                'visual_grammar':['approved-front-door','watch-for-the-dawn','dore-engraving','responsive-city-grid','5:8-journal','central-gate','archival-print']
            })
        return super().do_GET()
