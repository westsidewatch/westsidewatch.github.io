...
class H(visual.H):
    ...
    def do_GET(self):
        ...
        if path=='/':return self.send_bytes(200,home_preview().encode('utf-8'),'text/html; charset=utf-8')
        if path=='/editor':
            active=(q.get('page') or ['homepage'])[0]
            if active not in multipage_wysiwyg.SUPPORTED:active='homepage'
            return self.send_bytes(200,multipage_wysiwyg.render_editor(active).encode('utf-8'),'text/html; charset=utf-8')
