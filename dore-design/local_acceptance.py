...
health=j('/api/health');assert health.get('ok') and str(health.get('version',''))>='0.8',health
...
print(json.dumps({'ok':True,'code':'DORE_DESIGN_LOCAL_ACCEPTANCE_PASS','version':health.get('version'),'page_count':v2['page_count'],'revision':v2['revision'],'same_artifact_mutation':True,'resident_render':True,'resident_verify':True,'cleanup':True},ensure_ascii=False))
