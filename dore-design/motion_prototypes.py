...
def _page():
    return {
        ...
        'design_experiment':{
            ...
            'principles':['no-light-effects','hover-is-primary-on-desktop','tap-is-fallback-on-touch','D-codrops-original-reference','E-four-living-currents-to-temporary-puzzle','grid-never-visible-at-rest','dore-images-at-rest','continuous-selection-without-return','four-pieces-reconstruct-one-image','no-center-void'],
            'acceptance':['four-horizontal-currents-visible-at-rest','different-speeds','weighted-8:5-spans','dore-images-visible-at-rest','desktop-hover-forms-preview-without-click','hovering-next-item-switches-preview','touch-tap-still-works','currents-keep-moving','preview-does-not-block-hover-field','puzzle-has-no-black-center-hole','reversible']
        }
    }
...
(function(){
 ...
 function activate(c){
   if(!c||c===current)return;current=c;cards.forEach(function(x){x.classList.remove('selected')});c.classList.add('selected');
 ...
 cards.forEach(function(c){
   var u=url(c.dataset.dore,1100);if(u)c.style.backgroundImage='url("'+u+'")';
   if(hoverMode){c.addEventListener('mouseenter',function(){clearTimeout(timer);timer=setTimeout(function(){activate(c)},55)})}
   c.addEventListener('click',function(e){if(hoverMode){e.preventDefault();return}e.stopPropagation();activate(c)});
 });
 document.getElementById('back').onclick=function(e){e.stopPropagation();clearTimeout(timer);f.classList.remove('active');cards.forEach(function(x){x.classList.remove('selected')});current=null};
