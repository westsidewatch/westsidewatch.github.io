    ...
    return {
        ...
        'design_experiment': {
            ...
            'principles': [
                ...
                'E-four-living-currents-to-temporary-puzzle',
                'grid-never-visible-at-rest',
                'dore-images-at-rest',
                'continuous-selection-without-return',
                'four-pieces-reconstruct-one-image',
                'no-center-void',
                'tap-not-hover',
            ],
            'acceptance': [
                'four-horizontal-currents-visible-at-rest',
                'different-speeds',
                'weighted-8:5-spans',
                'dore-images-visible-at-rest',
                'click-forms-temporary-puzzle',
                'second-click-switches-content-directly',
                'puzzle-has-no-black-center-hole',
                'selected-content-identity-preserved',
                'reversible',
            ],
        },
    }
...
(function(){
 ...
 function activate(c){
   document.querySelectorAll('.flow-card').forEach(function(x){x.classList.remove('selected')});c.classList.add('selected');
   var img=url(c.dataset.dore,2000),pieces=document.querySelectorAll('.piece');
   f.classList.remove('active');void f.offsetWidth;
   pieces.forEach(function(p){p.style.backgroundImage='url("'+img+'")'});
 ...
 document.querySelectorAll('.flow-card').forEach(function(c){var u=url(c.dataset.dore,1100);if(u)c.style.backgroundImage='url("'+u+'")';c.addEventListener('click',function(){activate(c)})});
 document.getElementById('back').onclick=function(){f.classList.remove('active');document.querySelectorAll('.flow-card').forEach(function(x){x.classList.remove('selected')})};
 document.getElementById('enter').onclick=function(){document.getElementById('dest').classList.add('on')};
 document.getElementById('destBack').onclick=function(){document.getElementById('dest').classList.remove('on')};
})();</script></body></html>'''
