...
Uses the locked approved homepage as the read-only control specimen, extracts
Westside tokens, writes a real Storybook research specimen, builds Storybook,
and records evidence. The approved homepage is never modified.
"""
...
tokens = {k: cssvar(k, v) for k, v in {
    'paper':'#f2eee4','ink':'#1e211f','night':'#0b2639','night-2':'#071a28',
    'gold':'#b39a47','gold-soft':'#d0bd78'
}.items()}
...
'''
story.write_text(template.replace('__TOKENS__', json.dumps(tokens, ensure_ascii=False)), encoding='utf-8')
