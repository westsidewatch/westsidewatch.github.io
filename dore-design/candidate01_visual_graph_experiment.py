...
.living-current-band{gap:clamp(12px,1.45vw,28px)!important;align-items:center!important}.living-current-band .product{position:relative;overflow:hidden;flex:1 1 0!important;min-width:0!important;aspect-ratio:8/5!important;max-width:none!important;transition:transform .65s cubic-bezier(.2,.75,.2,1),opacity .4s ease,filter .4s ease}.living-current-band .product[data-editorial-weight="1"],.living-current-band .product[data-editorial-weight="2"],.living-current-band .product[data-editorial-weight="3"]{flex-grow:1!important;flex-basis:0!important}.living-current-band .product[data-editorial-shape="portrait"],.living-current-band .product[data-editorial-shape="landscape"],.living-current-band .product[data-editorial-shape="square"]{aspect-ratio:8/5!important;max-width:none!important}.living-current-band:has(.product.is-contemplated) .product:not(.is-contemplated){opacity:.38;filter:saturate(.65)}.living-current-band .product.is-contemplated{flex-grow:1!important;z-index:8;transform:none}.lw-editorial-tag{position:absolute;z-index:9;left:.7rem;top:.65rem;padding:.25rem .42rem;background:rgba(8,16,20,.72);color:#CEBD74;font:600 9px/1 ui-monospace,monospace;letter-spacing:.12em;pointer-events:none}.lw-inline-reader{position:absolute;z-index:10;inset:0;display:grid;grid-template-columns:minmax(38%,.72fr) minmax(0,1.28fr);background:rgba(8,16,20,.94);color:#f7f1df;opacity:0;pointer-events:none;transition:opacity .28s ease;overflow:hidden}.product.is-reading .lw-inline-reader{opacity:1;pointer-events:auto}.lw-reader-image{background-size:cover;background-position:center}.product[data-editorial-has-visual="false"] .lw-inline-reader{grid-template-columns:1fr}.product[data-editorial-has-visual="false"] .lw-reader-image{display:none}.lw-reader-flow{display:flex;overflow-x:auto;overscroll-behavior-x:contain;scroll-snap-type:x proximity;scrollbar-width:none}.lw-reader-flow::-webkit-scrollbar{display:none}.lw-reader-panel{flex:0 0 min(76%,420px);padding:clamp(16px,2vw,34px);display:flex;flex-direction:column;justify-content:flex-end;border-left:1px solid rgba(206,189,116,.25);scroll-snap-align:start}.lw-reader-panel .eyebrow{color:#CEBD74;font:600 9px/1.3 ui-monospace,monospace;letter-spacing:.14em}.lw-reader-panel h3{font:400 clamp(25px,2.6vw,46px)/.9 "Cormorant Garamond","Noto Serif TC",serif;margin:.45rem 0}.lw-reader-panel p{font:400 clamp(12px,.95vw,16px)/1.55 "Noto Serif TC",serif;margin:0}.lw-reader-panel.next{justify-content:center;color:#CEBD74}.lw-reader-panel.next:after{content:'CLICK → ENTER WORLD';font:600 9px/1 ui-monospace,monospace;letter-spacing:.12em;margin-top:1rem}@media(max-width:900px){.living-current-band .product{aspect-ratio:8/5!important}.lw-inline-reader{grid-template-columns:1fr}.lw-reader-image{display:none}.lw-reader-panel{flex-basis:86%}}@media(prefers-reduced-motion:reduce){.living-current-band .product,.lw-inline-reader{transition:none!important}}
</style>'''

FOCUS_STABILIZER=r'''<script id="candidate-01-focus-stabilizer">(()=>{
 let armed=false,lastPhase='';
 const stage=()=>document.querySelector('.products');
 const mosaicBounds=()=>{const pieces=[...document.querySelectorAll('.dore-mosaic-piece')];if(!pieces.length)return null;const rs=pieces.map(p=>p.getBoundingClientRect());const left=Math.min(...rs.map(r=>r.left)),top=Math.min(...rs.map(r=>r.top)),right=Math.max(...rs.map(r=>r.right)),bottom=Math.max(...rs.map(r=>r.bottom));return {left,top,right,bottom}};
 const inside=(e,r)=>r&&e.clientX>=r.left&&e.clientX<=r.right&&e.clientY>=r.top&&e.clientY<=r.bottom;
 const guard=e=>{const s=stage(),phase=s?.dataset.motionPhase||'';if(phase!==lastPhase){if(!['arrest','assemble','focus'].includes(phase))armed=false;lastPhase=phase}if(phase==='arrest'||phase==='assemble'){e.stopPropagation();return}if(phase==='focus'&&!armed){const r=mosaicBounds();if(inside(e,r)){armed=true;return}e.stopPropagation()}};
 window.addEventListener('pointermove',guard,true);
 window.addEventListener('mousemove',guard,true);
})();</script>'''
...
def focus_screen(screen_no,labels):
    ...
    doc=motion._install_living_current(codrops_site_8x5.render(edit=False));doc=doc.replace('</head>',motion._CANDIDATE_FOCUS_EMBED_STYLE+STYLE+'</head>',1);doc=doc.replace('</body>',motion._movement_labels_script(labels)+FOCUS_STABILIZER+script(row_names)+'</body>',1);srcdoc=html_lib.escape(doc,quote=True)
