...
def render_snapshot(snapshot,edit=False):
    ...
    page=snapshot.get('page') or {};canvas=page.get('canvas') or {};width=_css_num(canvas.get('width',canvas.get('w',page.get('width',1440))));height=_css_num(canvas.get('height',canvas.get('h',page.get('height',900))));nodes=[]
    for node in page.get('nodes') or []:
        ...
        style+=_extra_style(node);text=escape(_node_text(node));attrs=f'data-d2-node="{nid}" data-id="{nid}" data-kind="{kind}"' if edit else f'data-id="{nid}" data-kind="{kind}"';nodes.append(f'<div {attrs} style="{";".join(style)}">{text}</div>')
