...
export async function onRequestPatch({request,env}){
  ...
  await env.DORE_SENSORY.prepare('UPDATE sensory_signals SET state=?1,research_task=COALESCE(?2,research_task),brain_node=COALESCE(?3,brain_node),error=?4,claimed_at=CASE WHEN ?1=\'RESEARCHING\' AND claimed_at IS NULL THEN ?5 ELSE claimed_at END,updated_at=?5 WHERE id=?6').bind(next,body?.research_task||null,body?.brain_node||null,body?.error||null,now,id).run();
