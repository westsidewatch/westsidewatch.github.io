...
export async function onRequestPost({request,env}){
  ...
  if(body?.action==='execute-caption-acquisition'){
    ...
    try{
      ...
      return json({ok:result.ok,schema:'dore.video-subtitle.v4',execution:persisted,boundary:'Caption acquisition uses only caption tracks advertised by the supported video source. No subtitle text is invented. Audio transcription remains a separate executor stage.'},http);
  ...
  const job={schema:'dore.video-subtitle.v4',url:rawUrl,canonical_url:canonicalUrl,resource_id:resourceId,target_language:targetLanguage,mode,created_at:createdAt,intent:'video_to_corrected_accessible_subtitle',pipeline:['identify-video','canonicalize-deduplicate','resolve-library-resource','obtain-caption-or-audio','transcribe-if-needed','dore-proofread','translate-if-needed','scripture-align','human-review','library-feedback','learning-feedback']};
...
export async function onRequestGet({request,env}){
  ...
  if(id&&env?.DORE_SENSORY){try{
    ...
    if(!row)return json({ok:false,error:'job_not_found'},404);
    return json({ok:true,schema:'dore.video-subtitle.v4',job:row});
  }catch(e){console.error('video subtitle job lookup',e);return json({ok:false,error:'job_lookup_failed'},500)}}
  return json({ok:true,schema:'dore.video-subtitle.v4',status:'caption-acquisition-executor-connected',pipeline:'video URL → canonicalize/deduplicate → Liming resource → advertised caption acquisition → audio transcription if needed → Doré proofread → translation → Scripture alignment → human review → library + learning feedback',executor:'youtube-advertised-caption-acquisition'});
}
