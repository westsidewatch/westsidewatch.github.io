...
def diagnose(request: dict[str, Any], observation: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    if "source" in families: roles.append("source-discovery")
    if "publishing" in families: roles.append("publishing-production")
    failure_domains = list(observation.get("failure_domains") or [])
    if observation.get("beautiful") is False: failure_domains.append("aesthetic-admission-failed")
    if observation.get("visual_motivation") is False: failure_domains.append("unmotivated-visual-state")
    return {"schema":"dore.capability-gap.v0","surface":surface,"beauty_first":beauty_first,"needed_families":list(dict.fromkeys(families)),"needed_roles":list(dict.fromkeys(roles)),"failure_domains":list(dict.fromkeys(failure_domains)),"principle":"beauty-is-an-admission-boundary-not-a-late-optimization" if beauty_first else "smallest-sufficient-loadout"}


def discover(gap: dict[str, Any], limit: int = 12) -> list[dict[str, Any]]:
    families=set(gap.get("needed_families") or []);needed_roles=set(gap.get("needed_roles") or []);scored=[]
    for capability in _registry():
        terms=_terms(capability);roles=_roles_for(capability)&needed_roles;score=len(roles)*12
        for family in families: score+=len(terms&SIGNALS.get(family,set()))*2
        if score: scored.append((score,capability,roles))
    scored.sort(key=lambda item:(-item[0],item[1].get("id","")))
    return [{"id":c["id"],"score":score,"roles":sorted(roles),"service":c.get("service"),"type":c.get("type"),"load":c.get("load","deferred"),"network":c.get("network")} for score,c,roles in scored[:limit]]


def assemble(gap: dict[str, Any], candidates: list[dict[str, Any]], max_weapons: int = 6) -> dict[str, Any]:
    chosen=[];covered_roles=set();needed_roles=list(gap.get("needed_roles") or [])
    for candidate in candidates:
        roles=set(candidate.get("roles") or [])
        if roles-covered_roles: chosen.append(candidate);covered_roles|=roles
        if len(chosen)>=max_weapons or all(role in covered_roles for role in needed_roles): break
    unresolved=[role for role in needed_roles if role not in covered_roles]
    return {"schema":"dore.capability-loadout.v0","beauty_first":bool(gap.get("beauty_first")),"weapons":chosen,"covered_roles":sorted(covered_roles),"unresolved_roles":unresolved,"requires_visual_admission":bool(gap.get("beauty_first")),"admission_rule":"real-raster-must-be-beautiful-and-visually-motivated" if gap.get("beauty_first") else "task-evidence-required"}


def plan(request: dict[str, Any], observation: dict[str, Any] | None = None) -> dict[str, Any]:
    gap=diagnose(request,observation);candidates=discover(gap);loadout=assemble(gap,candidates)
    return {"schema":"dore.capability-arsenal.v0","gap":gap,"candidates":candidates,"loadout":loadout,"evolution_required":bool(loadout["unresolved_roles"])}
