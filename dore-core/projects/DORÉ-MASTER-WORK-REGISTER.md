...
| 17A | CONV-MEM-V1 | Scoped Conversation Memory Layer | ACTIVE_PARALLEL / IMPLEMENTING | D1 schema/scoped-recent GET/dedupe exist, but current POST now hard-requires `DORE_MEMORY_ARCHIVE`, `DORE_MEMORY_VECTOR` and Workers `AI`, writing archive+embedding before D1; project id may still fall back to `unscoped`; semantic recall, rollback, namespace-collision and production isolation are not verified. Evidence: `DORÉ-CONVERSATION-MEMORY-EVIDENCE-LEDGER-2026-08-27.md` | Through the current D1+R2+AI+Vectorize path, run a production-safe two-conversation/two-project fixture proving exact replay, R2 recovery, scoped vector recall, zero cross-talk, namespace safety, rollback behavior and free-first cost/availability before broader integration |
