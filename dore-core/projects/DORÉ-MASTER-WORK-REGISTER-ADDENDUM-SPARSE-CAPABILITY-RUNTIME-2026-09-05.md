...
An eighth visual-state submilestone is now real at commit `48e0f326dac3f0374abbaddbba5b0656108f18ea`: accepted generated image artifacts can compile into renderer-neutral `DesignAsset` records plus ordinary `DesignImageShape` records with stable `asset_id`, checksum/provenance and canonical geometry/fit/role. Tests verify the artifact→asset/shape contract and explicitly verify that persistent native state does not contain Konva, Polotno or tldraw identity. The retained canvas architecture therefore makes application/workspace JSON authoritative, treats canvas nodes as interactive projections, keeps deterministic export separate, and preserves canvas-engine replaceability. This is `VERIFIED_COMPLETE_SUBMILESTONE` state-contract evidence, not live interactive-canvas or real-Westside-asset acceptance. Manual Image→Design download/upload and renderer-owned workspace serialization are now superseded default directions.

A ninth Design 2.0 product submilestone is now real through head `1e9a9edf633c4e51b060fac57c023ad819bb368e`: the native structured workspace exposes a coherent arrange family—horizontal/vertical alignment, center-to-canvas, distribute, fixed-gap stack, z-order, group and ungroup—and the complete command payload reaches the canonical `/api/workspace` persistence surface. Focused contract coverage plus a geometry/persistence smoke check pin the operation family. This is `ACTIVE_PARALLEL / VERIFIED_COMPLETE_SUBMILESTONE` repository evidence, not live-product acceptance. Manual coordinate-only arrangement is superseded as the default workflow for this implemented operation family, while mixed-size/nested-group/hidden-locked/save-reload interactive acceptance remains open. Evidence: `DORÉ-DESIGN2-ARRANGE-CAPABILITY-EVIDENCE-LEDGER-2026-09-05.md`.
...
- one real purpose-built Westside asset (prefer Doré-derived light texture or Bethlehem-star grammar rather than an original Doré artwork) completing generation → real resident/provider render → fetched durable bytes + checksum/provenance → real vision observations → critic/correction if needed → accepted artifact → renderer-neutral `DesignAsset` + `DesignImageShape` → typed Design application/manipulation with persisted identity/provenance and before/after acceptance evidence → deterministic export/verification;
- latest-head capability-runtime CI PASS including the local A2A bridge, Companion Native contract, Companion 1.0 installable shell, Image iteration/handoff/resident-pipeline, resident-autorun and asset/shape-state coverage;
...
- one bounded interactive Design 2.0 arrange acceptance packet proving mixed-size multi-select align/distribute, canvas-center/fixed-gap stack, z-order/group state through save/reload, hidden/locked semantics and a before/after export;
...
For Doré Image quality work, one-shot generation as the default final-production model is superseded by the bounded correction-loop direction where visual acceptance matters. Provider-returned image references should likewise not remain ephemeral when an asset enters Doré's production/evaluation loop: the governing path is durable bytes + checksum/provenance + real review. Resident autorun now provides the bounded zero-terminal seam that can consume a queued visual job once local renderer configuration is present. Where Image hands an accepted asset to Design, free-form conversational transfer and manual file shuttling are superseded by the typed image-patch plus renderer-neutral asset/shape contract. Canvas/rendering libraries are views over Doré-owned state and must not become the persistent workspace authority. Design 2.0's native arrange family now adds semantic geometry operations above that same canonical state; defaulting back to manual coordinate-only editing for supported arrange tasks is superseded, while live interactive acceptance remains evidence-gated. These are architectural directions, not claims that the real visual-production loop is already complete.
