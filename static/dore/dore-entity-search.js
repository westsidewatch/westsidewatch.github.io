...
function renderEntities(mention,entities,mode='entity'){
 ...
 let intro='';if(mode==='count')intro=`<article class="result-card"><header><strong>${esc(mention)}</strong><span>全經 Entity aggregation</span></header><p>目前來源個體化資料辨識出 <strong>${entities.length}</strong> 個不同人物候選。這是依來源的個體化記錄計數；若身份合併存在學術或傳統爭議，Doré 不會把爭議偷偷消除。</p><footer><span>BW-1 entity-count</span><span>STEPBible TIPNR · canonical attestations</span></footer></article>`;
...
async function handler(ev){
 ...
 if(countMention){const es=exactEntities(countMention,'person');if(es.length){renderEntities(countMention,es,'count');history.replaceState(null,'',`#q=${encodeURIComponent(raw)}`);return}bypass=true;$('#search-form')?.requestSubmit();return}
