...
function installConversationUI(){
 ...
 .dore-chat{display:flex;flex-direction:column;gap:1.05rem;padding:1rem 0 2rem}
...
function renderConversationShell(messages=[]){const box=$('#results'),count=$('#result-count');if(!box)return;box.innerHTML=`<div class="dore-chat">${messages.map(messageHTML).join('')}</div>`;if(count)count.textContent=messages.length?`${messages.length} messages`:'新對話';$('#results-wrap')?.scrollIntoView({behavior:'smooth',block:'start'})}
function appendMessage(role,content,pending=false){let chat=$('#results .dore-chat');if(!chat){renderConversationShell([]);chat=$('#results .dore-chat')}const el=document.createElement('article');el.className=`dore-msg ${role}${pending?' pending':''}`;el.innerHTML=(role==='assistant'?'<div class="who">DORÉ</div>':'')+`<div>${role==='assistant'?markdown(content):esc(content)}</div>`;chat.appendChild(el);el.scrollIntoView({behavior:'smooth',block:'end'});return el}
...
async function converse(detail){appendMessage('user',detail.query);const wait=appendMessage('assistant','Doré 回應中…',true);const count=$('#result-count');if(count)count.textContent='Doré 回應中';try{if(!(await localReady()))throw new Error('Doré Local 未連線');const r=await fetch(LOCAL_CHAT,{method:'POST',headers:{'content-type':'application/json',accept:'application/json'},body:JSON.stringify({message:detail.query,conversation_id:detail.conversation_id,project_id:PROJECT_ID})});const d=await r.json();if(!r.ok||!d?.ok)throw new Error(d?.detail||d?.error||`HTTP ${r.status}`);wait.classList.remove('pending');wait.innerHTML=`<div class="who">DORÉ</div><div>${markdown(d.reply||'')}</div>`;if(count)count.textContent='已保存 · Local';window.dispatchEvent(new CustomEvent('dore:conversation-response',{detail:d}));return d}catch(e){wait.classList.remove('pending');wait.innerHTML=`<div class="who">DORÉ</div><div>${esc('Doré 對話目前不可用。 '+String(e?.message||e))}</div>`;if(count)count.textContent='對話失敗';return null}}
...
window.DoreSearchRuntime={version:'7.0.0',snapshot,conversationId,setConversationId,newConversation,converse,aiMode,setAiMode,isOpenCommand,isCloseCommand,localReady,loadCurrentConversation,loadConversationList,openConversationMode,closeConversationMode};
init();
})();
