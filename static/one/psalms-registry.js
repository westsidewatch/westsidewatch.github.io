/* 詩篇 ONE：完整性門檻與書卷註冊。
 * 僅當 150 篇研究資料與 150 篇插圖全部完整時，才把詩篇暴露給 ONE。
...
(() => {
  ...
  const missingStudies=[],incompleteStudies=[],missingIllustrations=[];
  ...
  for(let number=1;number<=expected;number+=1){
    ...
    if(!nonEmptyString(study.illustration?.src)||!nonEmptyString(study.illustration?.source))missingIllustrations.push(number);
  }
  ...
  const ready=missingStudies.length===0&&incompleteStudies.length===0&&missingIllustrations.length===0;
  ...
  if(ready){
    ...
    console.error(`[ONE Psalms] registration blocked: missing=${missingStudies.join(",")||"none"}; incomplete=${incompleteStudies.join(",")||"none"}; illustrations=${missingIllustrations.join(",")||"none"}.`);
  }
})();
