/* Isaiah maps: use ONE's existing map renderer schema. */
(() => {
  "use strict";
  const I=window.ONE_DATA?.isaiah;if(!I?.chapterStudies)return;
  const holy=(id,title,reference,guide,places)=>({
    mapId:id,
    reference,
    title,
    guide,
  ...
  const maps={
    ...
    babylon:holy(76,"王下圖八（076）巴比倫帝國","以賽亞書 39–48","從巴比倫使者事件轉入被擄與歸回視野；用地圖掌握耶路撒冷與巴比倫之間的帝國尺度。",["耶路撒冷","巴比倫","米所波大米","歸回方向"])
  };
  ...
  assign([28,29,30,31,32,33,34,35],maps.assyria);
  ...
  I.mapPolicy={source:"聖光聖經地理",rule:"只在地理、帝國、戰爭或遷徙背景能實質幫助理解時配置地圖；資料欄位完全遵循 ONE 現有 map renderer。"};
})();