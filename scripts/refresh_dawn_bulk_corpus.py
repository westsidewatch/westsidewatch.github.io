...
def main() -> int:
    ...
    print(json.dumps({'schema':'dawn.bulk-corpus-refresh.v1','sourceWorks':source['metrics']['works'],'chineseWorks':source['metrics']['chineseMatchedWorksAdded'],'queueWorks':queue['deduplicatedWorks'],'authorityBackedWorks':queue['authorityBackedWorks'],'canonicalWorks':index['workCount']},ensure_ascii=False))
    return 0
