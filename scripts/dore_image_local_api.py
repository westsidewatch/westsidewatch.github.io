...
def _comfy_health() -> tuple[bool, dict]:
    cfg = _raw_config()
    if cfg.get("renderer") not in (None, "comfyui") or not _config_path().exists():
        return False, {"config": bool(cfg), "detail": "not-comfyui"}
    try:
        config = load_resident_image_config(_config_path())
...
def _renderer_health() -> tuple[str | None, dict]:
    comfy, meta = _comfy_health()
    if comfy:
        return "comfyui", meta | {"model_backed": True}
    sd_ok, sd_meta = sd_cpp_health(_config_path())
    if sd_ok:
        return "stable-diffusion.cpp", sd_meta | {"model_backed": True}
    raw = _raw_config()
    return None, {
        "config": bool(raw),
        "detail": sd_meta.get("detail") if raw.get("renderer") == "stable-diffusion.cpp" else meta.get("detail", "resident-config-missing"),
        "model_backed": False,
        "configured_renderer": raw.get("renderer"),
    }


def _safe_artifact(artifact: dict, asset_url: str) -> dict:
    safe = {k: artifact[k] for k in ("id", "sha256", "bytes", "mime_type", "model") if k in artifact}
...
def _native_svg(command) -> dict:
    ...
    gold = "#CDB46A"; ink = "#1E1E1B"; paper = "#F5F0E5"
    lines = []
    for i in range(11):
        x1 = 90 + ((seed >> (i % 24)) % 280); y1 = 120 + i * 92
        x2 = 1110 - ((seed >> ((i + 7) % 24)) % 260); y2 = 130 + i * 94
        lines.append(f'<path d="M{x1} {y1} Q600 {y1-70+((seed+i)%140)} {x2} {y2}"/>')
    circles = []
    for i in range(5):
        cx = 250 + ((seed >> (i * 3)) % 700); cy = 300 + ((seed >> (i * 5 + 2)) % 850); r = 30 + ((seed >> (i * 4 + 1)) % 110)
    ...
    raw = svg.encode("utf-8"); sha = hashlib.sha256(raw).hexdigest(); name = f"native-{sha[:20]}.svg"; path = out / name; path.write_bytes(raw)
    return {"id": f"sha256:{sha}", "sha256": sha, "bytes": len(raw), "mime_type": "image/svg+xml", "uri": str(path), "renderer": "dore-native-svg"}


class Handler(BaseHTTPRequestHandler):
    server_version = "DoreImageLocal/1.4"
    ...
    def _json(self, status: int, payload: dict) -> None:
        raw = json.dumps(payload, ensure_ascii=False).encode("utf-8"); self.send_response(status); self._cors(); self.send_header("Content-Type", "application/json; charset=utf-8"); self.send_header("Cache-Control", "no-store"); self.send_header("Content-Length", str(len(raw))); self.end_headers(); self.wfile.write(raw)

    def do_OPTIONS(self) -> None: self.send_response(204); self._cors(); self.end_headers()

    def do_GET(self) -> None:
        parsed = urlparse(self.path)
        if parsed.path == "/health":
            mode, meta = _renderer_health()
            raw = _raw_config()
            self._json(200, {"ok": True, "node": "dore-image-local", "renderer": bool(mode), "renderer_mode": mode or "dore-native-svg", "model_backed": bool(mode), "native_svg": True, "comfyui": mode == "comfyui", "stable_diffusion_cpp": mode == "stable-diffusion.cpp", "detail": meta.get("detail"), "endpoint": meta.get("endpoint"), "model": meta.get("model") or raw.get("model"), "model_bytes": meta.get("model_bytes"), "config": meta.get("config", False), "config_path": str(_config_path())})
            return
        if parsed.path == "/asset":
            ...
            try:
                raw = _raw_config(); output_dir = raw.get("output_dir")
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            with _LOCK:
                ...
                if mode == "comfyui":
                    job_dir = ROOT / "dore-image" / "jobs"; job_dir.mkdir(parents=True, exist_ok=True); job_path = job_dir / "search-ai-next.json"
                    ...
                    prompt = (command.brief or command.subject or command.message).strip()
                    artifact = sd_cpp_generate(_config_path(), prompt=prompt, seed=command.seed, output_dir=ROOT / "dore-image" / "generated")
                    renderer = "stable-diffusion.cpp"; recipe = {"family": "DORÉ/SDCPP", "seed": command.seed, "model": artifact.get("model")}; prompt_id = None
                else:
                    artifact = _native_svg(command); renderer = "dore-native-svg"; recipe = {"family": "DORÉ/NATIVE-EDITORIAL-EMERGENCY", "seed": command.seed}; prompt_id = None
            name = Path(str(artifact.get("uri", ""))).name
            if not name: raise RuntimeError("renderer returned no asset")
            asset_url = f"http://{HOST}:{PORT}/asset?name={name}"; safe = _safe_artifact(artifact, asset_url); safe["renderer"] = renderer
            self._json(200, {"ok": True, "capability": "image.generate", "message": command.message, "renderer": renderer, "model_backed": renderer in {"comfyui", "stable-diffusion.cpp"}, "artifact": safe, "recipe": recipe, "prompt_id": prompt_id, "asset_url": asset_url})
