 ...
 *
 * Persists approved Studio images as REAL repository binary files. It deliberately avoids
 * chat/base64 staging, browser reconstruction, data URLs and runtime fetch bridges.
 *
 * One plate:
 *   node scripts/dore/publish-studio-asset.mjs \
 *     --input /absolute/path/lam03.png --book 25 --chapter 3 \
 *     --asset-id LAM-03-DORE-STUDIO-001 --title "Lamentations III" \
 *     --scripture "Lamentations 3" --filename lamentations-03-dore-studio-v2.png
 *
 * Batch:
 *   node scripts/dore/publish-studio-asset.mjs --manifest /path/jobs.json
 *
 * This tool changes only the working tree. Git commit/push/PR/merge are intentionally
 * outside the tool, allowing 10–30 approved plates to share one commit and one PR.
 */
...
function dimensions(buffer, kind) {
  if (kind === 'png' && buffer.length >= 24) return {width:buffer.readUInt32BE(16),height:buffer.readUInt32BE(20)};
  if (kind === 'webp' && buffer.length >= 30 && buffer.subarray(12,16).toString('ascii') === 'VP8X') {
...
async function atomicCopy(source, target) {
  ...
  if (src.length !== tmp.length || sha256(src) !== sha256(tmp)) fail(`atomic copy verification failed: ${target}`);
  await rename(temp, target);
}

function registryLines(job, publicSrc, digest, bytes) {
  const note = `${job.note} SHA-256 ${digest}; ${bytes} bytes.`;
  return {
    asset:`  registerAsset(${q(job.assetId)},{src:${q(publicSrc)},alt:${q(job.alt)},title:${q(job.title)},source:${q(publicSrc)},artist:"Westside Watch Engraving Studio · Doré continuation",origin:"ONE_STUDIO_DORE_CONTINUATION",palette:"MONOCHROME_ENGRAVING",scripture:${q(job.scripture)},approvedAt:${q(job.approvedAt)},note:${q(note)}});`,
    assign:`  registerChapter(${job.book},${job.chapter},${q(job.assetId)},{priority:"P7_ONE_STUDIO_FIXED",basis:"EDITORIAL_FINAL_NATIVE_BINARY"});`
  };
}
...
async function updateRegistry(job, publicSrc, digest, bytes, dryRun) {
  ...
  const assetNeedle = `registerAsset(${q(job.assetId)},`;
  const chapterNeedle = `registerChapter(${job.book},${job.chapter},`;
  const assetExists = text.includes(assetNeedle);
  const chapterExists = text.includes(chapterNeedle);
  if ((assetExists || chapterExists) && !job.replace) fail(`registry already contains ${job.assetId} or ${job.book}:${job.chapter}; use --replace only after editorial approval`);

  if (job.replace) {
    const lines = text.split('\n').filter(line => !line.includes(assetNeedle) && !line.includes(chapterNeedle));
    text = lines.join('\n');
  }
...
async function publish(job,{dryRun=false}={}) {
  ...
  if (!dryRun) {
    await atomicCopy(job.input,target);
  ...
  const receipt = {
    ...
    bytes:buffer.length,sha256:digest,dimensions:dimensions(buffer,format.kind),
...
async function main() {
  ...
  console.error(`[Doré Publisher] ${dryRun?'DRY RUN':'OK'}: ${receipts.length} asset(s). Commit binary files + one-studio-assets.js + receipts together.`);
}

main().catch(error => { if (!process.exitCode) process.exitCode=1; if (!String(error?.message||'').includes('[Doré Publisher]')) console.error(error); });
