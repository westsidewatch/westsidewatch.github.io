...
DEFAULT_ADMISSIONS = ROOT / 'data/dawn-publication-admissions'
...
def stable_dawn_work_id(seed: str) -> str:
    return f"dawn:{hashlib.sha256(seed.encode('utf-8')).hexdigest()[:20]}"


def is_wikisource_url(value: Any) -> bool:
...
def contains_forbidden_wikisource(value: Any) -> bool:
    """Final admission gate, independent of discovery and runtime pointer filtering."""
...
def assert_no_wikisource(value: Any, context: str) -> None:
    if contains_forbidden_wikisource(value):
        raise ValueError(f'forbidden Wikisource source at final canonical admission: {context}')


def local_runtime_pointer(value: Any) -> str | None:
    value = str(value or '').strip()
    lower = value.casefold()
    if not value or lower.startswith(('http://', 'https://', '//')) or any(t in lower for t in FORBIDDEN_RUNTIME_TOKENS):
        return None
    return value
...
def queue_work_id(item: dict) -> str:
    ...
    return authority or stable_dawn_work_id(str(item.get('queueId') or signature(item.get('title', ''), item.get('author', ''))))


def canonical_from_queue(item: dict) -> dict:
    assert_no_wikisource(item, f"queue:{item.get('queueId') or item.get('title') or 'unknown'}")
...
def ensure_surface_work(item: dict, work_id: str, works: dict[str, dict]) -> None:
    assert_no_wikisource(item, f'surface:{work_id}')
    if work_id in works:
        return
    work = item.get('work') or {}
...
def compile_storefront(storefront: dict, works: dict[str, dict], by_signature: dict[str, str]) -> dict:
    ...
    for shelf in storefront.get('shelves', []):
        ...
        for item in shelf.get('items', []):
            assert_no_wikisource(item, f"storefront:{shelf.get('id') or 'unknown'}")
...
def compile_multiwrite_surface(catalog: dict, works: dict[str, dict], by_signature: dict[str, str]) -> dict:
    ...
    for item in catalog.get('items', []):
        assert_no_wikisource(item, 'multiwrite-biblical-world')
        work_id = surface_item_work_id(item, by_signature)
        ensure_surface_work(item, work_id, works)
        ref = {'workId': work_id}
...
def load_publication_admissions(directory: Path) -> list[dict]:
    ...
    for path in sorted(directory.glob('*.json')):
        row = read_json(path)
        if row.get('schema') != 'dore.dawn-publication-admission.v1':
            raise ValueError(f'invalid publication admission: {path.name}')
        assert_no_wikisource(row, f'publication:{path.name}')
        result.append(row)
    return result
...
def apply_publications(works: dict[str, dict], dawn_surface: dict, admissions: list[dict]) -> None:
    ...
    if shelf is None:
        shelf = {'id': 'dore-publications', 'title': '多寫 · 正式出版', 'kind': 'canonical-publications', 'items': []}
        dawn_surface.setdefault('shelves', []).insert(0, shelf)
    existing = {r.get('workId') for r in shelf.get('items', [])}
    for admission in admissions:
        assert_no_wikisource(admission, f"publication:{(admission.get('work') or {}).get('workId') or 'unknown'}")
        runtime = {'work': admission.get('work'), 'edition': admission.get('edition'), 'surface': admission.get('surface')}
        serialized = json.dumps(runtime, ensure_ascii=False).casefold()
        if any(t in serialized for t in FORBIDDEN_RUNTIME_TOKENS):
            raise ValueError('forbidden publication runtime dependency')
        work = admission.get('work') or {}
        edition = admission.get('edition') or {}
        policy = admission.get('policy') or {}
        surface = admission.get('surface') or {}
        work_id = str(work.get('workId') or '')
        if not work_id or edition.get('workId') != work_id or (surface.get('ref') or {}).get('workId') != work_id:
            raise ValueError('publication identity mismatch')
        if policy.get('surfaceOwnsIdentity') is not False or policy.get('wikisource') != 'forbidden':
            raise ValueError('publication policy mismatch')
        artifacts = edition.get('artifacts') or {}
        for kind in ('cover', 'web', 'epub', 'pdf'):
            ptr = local_runtime_pointer(artifacts.get(kind))
            if not ptr:
                raise ValueError(f'invalid publication {kind} pointer: {work_id}')
            path = ROOT / ptr.lstrip('/')
            if not path.is_file():
                raise ValueError(f'missing publication artifact: {ptr}')
        canonical = works.get(work_id) or {'workId': work_id, 'authorityIds': {}, 'firstPublishYear': None, 'authorityBacked': False}
        canonical.update({
            'title': str(work.get('title') or canonical.get('title') or '').strip(),
            'authors': list(dict.fromkeys(work.get('authors') or canonical.get('authors') or [])),
            'languages': list(dict.fromkeys(work.get('languages') or canonical.get('languages') or [])),
            'edition': edition,
            'editions': [edition],
            'cover': {'pointer': f'dawn://cover/{work_id}', 'mode': 'published-artifact'},
            'readingPointer': artifacts.get('web'),
            'publicationOrigin': 'multiwrite',
        })
        works[work_id] = canonical
        if work_id not in existing:
            shelf['items'].insert(0, {'workId': work_id})
...
def assert_runtime_contract(index: dict, surfaces: list[dict]) -> None:
    ...
    for work_id, work in works.items():
        ...
        if not str((work.get('cover') or {}).get('pointer') or '').startswith('dawn://cover/'):
...
def build(queue: dict, storefront: dict, biblical_world: dict, admissions: list[dict] | None = None) -> tuple[dict, dict, dict]:
    ...
    for item in queue.get('items', []):
        # No silent filtering here. A forbidden legacy source is a build failure so it can
        # never disappear into canonical state unnoticed.
        assert_no_wikisource(item, f"queue:{item.get('queueId') or item.get('title') or 'unknown'}")
        record = canonical_from_queue(item)
        work_id = record['workId']
        works[work_id] = record
        sig = signature(record['title'], record['authors'][0] if record['authors'] else '')
        if sig != '::':
    ...
    apply_publications(works, dawn_surface, admissions or [])
...
def main() -> int:
    ...
    p.add_argument('--storefront', type=Path, default=DEFAULT_STOREFRONT)
    p.add_argument('--biblical-world', type=Path, default=DEFAULT_BIBLICAL_WORLD)
    p.add_argument('--admissions-dir', type=Path, default=DEFAULT_ADMISSIONS)
    p.add_argument('--index-out', type=Path, default=DEFAULT_INDEX)
    p.add_argument('--dawn-surface-out', type=Path, default=DEFAULT_DAWN_SURFACE)
    p.add_argument('--multiwrite-surface-out', type=Path, default=DEFAULT_MULTIWRITE_SURFACE)
    args = p.parse_args()
    ...
    admissions = load_publication_admissions(args.admissions_dir)
    ...
    print(json.dumps({
        ...
        'publicationAdmissions': len(admissions),
