...
const rows=Array.isArray(quality.chapters)?quality.chapters:[];
const byStatus=rows.reduce((acc,row)=>{const s=row.level||'UNKNOWN';acc[s]=(acc[s]||0)+1;return acc;},{});
const warningReasons={};
for(const row of rows){for(const issue of row.issues||[]){const key=`${issue.level||row.level}: ${issue.message||'unspecified issue'}`;warningReasons[key]=(warningReasons[key]||0)+1;}}
const topReasons=Object.entries(warningReasons).sort((a,b)=>b[1]-a[1]).slice(0,30);
const books={};
for(const row of rows){const key=`${String(row.bookNumber).padStart(2,'0')} ${row.book||''}`;books[key]||={PASS:0,WARNING:0,FAIL:0,total:0};books[key][row.level]=(books[key][row.level]||0)+1;books[key].total++;}

const report={generatedAt:new Date().toISOString(),executedScripts:executed,canon,schema:{ok:schema.ok,errors:schema.errors,warnings:schema.warnings},quality:{structuralOk:quality.structuralOk,needsReview:quality.needsReview,counts:quality.counts,byStatus,topReasons,books}};
fs.mkdirSync(path.join(root,'audit-output'),{recursive:true});
fs.writeFileSync(path.join(root,'audit-output','one-global-audit.json'),JSON.stringify(report,null,2));
let md=`# ONE Global Audit\n\n- Canon: **${canon.ok?'PASS':'FAIL'}** — ${canon.registeredBooks}/66 books, ${canon.registeredChapters}/1189 chapters\n- Schema: **${schema.ok?'PASS':'FAIL'}** — ${schema.errors.length} errors, ${schema.warnings.length} normalization warnings\n- Quality: **${quality.structuralOk?'STRUCTURALLY PASS':'FAIL'}** — PASS ${byStatus.PASS||0}, WARNING ${byStatus.WARNING||0}, FAIL ${byStatus.FAIL||0}\n\n## Top issues\n\n`;
