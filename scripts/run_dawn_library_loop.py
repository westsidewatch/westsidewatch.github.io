...
ZH_PROMO=ROOT/'reports/DAWN-LIBRARY-CHINESE-PROMOTION.json'
...
eng_disc=load(ENG_DISC);zh_disc=load(ZH_DISC);eng_promo=load(ENG_PROMO);zh_promo=load(ZH_PROMO)
pipeline={
    'chineseDiscoveredThisRun':zh_disc.get('newDiscovered',0),
    'chinesePromotedThisRun':zh_promo.get('promoted',0),
    'chinesePromotedTitles':[x.get('title') for x in zh_promo.get('items',[])],
    'englishDiscoveredThisRun':eng_disc.get('newDiscovered',0),
    'englishPromotedThisRun':eng_promo.get('promoted',0),
    'englishPromotedTitles':[x.get('title') for x in eng_promo.get('items',[])],
}
