...
REPORT=ROOT/'reports/DAWN-LIBRARY-CHINESE-PROMOTION.json'
...
for x in resolver.get('items',[]):
    ...
    item={
        ...
        'rights':{'status':'public-domain','jurisdiction':'USA','declaredBy':'Project Gutenberg','provenanceRequired':True,'evidenceUrl':edition.get('metadataUrl')},
        'relations':['聖經世界','中文公版'],
        ...
        'provenance':{'origin':'autonomous-discovery','languageLane':'zh','source':'project-gutenberg','discoveredAt':x.get('discoveredAt'),'verifiedAt':x.get('resolvedAt'),'catalogedAt':now}
    }
...
report={'schema':'dawn.library.chinese-promotion.report.v1','generatedAt':now,'verifiedInput':sum(1 for x in resolver.get('items',[]) if x.get('stage')=='verified'),'promoted':len(added),'items':added,'catalogTotal':len(catalog['items']),'origin':'autonomous-discovery','source':'project-gutenberg','contentDownloaded':False}
