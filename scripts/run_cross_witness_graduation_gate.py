...
EXPECTED_SCOPE = {
    ...
    # Rahlfs/CenterBLC maps 37 Protestant-canon books directly here. Ezra-Nehemiah
    # remain under source-specific 1Esdr/2Esdr identity pending an explicit,
    # evidence-backed versification map; they must not be force-normalized.
...
def main() -> None:
    try:
        ...
        seen={item.get("witness_id"):int(item.get("books",-1)) for item in audit.get("witnesses",[]) if item.get("witness_id")}
        ...
        result={"schema":"dore.cross-witness-graduation-gate.v0.2","verdict":verdict,"milestone":milestone,"engineering_failures":failures,"engineering_suspect_count":0 if not failures else len(failures),"research_exception_count":research,"category_counts":counts,"scope_invariants":{"expected":EXPECTED_SCOPE,"observed":seen},"rules":{"PASS":"all ingestion/inventory/alignment invariants pass and no structural reference exceptions remain","RESEARCH_EXCEPTIONS_ONLY":"engineering invariants pass; all remaining reference differences are preserved as research phenomena","FAIL":"pipeline, inventory, canonical-scope, or identity invariant failed"},"research_boundary":"Single-witness, anchor-only, multi-witness, translation-divergence and source-specific reference patterns are not automatically engineering defects or textual judgments."}
    except Exception as exc:
        result={"schema":"dore.cross-witness-graduation-gate.v0.2","verdict":"FAIL","milestone":"CROSS_WITNESS_GRADUATION_FAILED","engineering_failures":[f"{type(exc).__name__}: {exc}"]}
