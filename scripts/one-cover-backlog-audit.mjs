...
const progress=JSON.parse(fs.readFileSync(path.join(oneDir,'ONE-PRODUCTION-PROGRESS-TEMP.json'),'utf8'));
const acceptedBacklogCeiling=Number(progress?.baseline?.missing);
if(!Number.isInteger(acceptedBacklogCeiling)||acceptedBacklogCeiling<0)throw new Error('Invalid Missing Plate ceiling in ONE-PRODUCTION-PROGRESS-TEMP.json');
...
if(report.backlogGuard.status==='FAIL')throw new Error(`Missing Plate regression: accepted ceiling ${report.backlogGuard.acceptedCeiling}, got ${report.backlogGuard.current}`);
