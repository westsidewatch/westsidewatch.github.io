...
for(const file of shardFiles){
 ...
 for(const w of works){
  ...
  const rawMedium=[w.medium,w.mediaType,w.resourceType,w.format,w.type,w.kind].flat().filter(Boolean).map(String); if(!rawMedium.length)stats.missing.medium++; rawMedium.forEach(x=>inc(stats.mediums,x));
  const pointers=[w.pointer,w.pointers,w.url,w.urls,w.source,w.sources,w.access].flat().filter(Boolean); const pointerText=JSON.stringify(pointers); const mediumText=rawMedium.join(' '); const titleText=[w.title,w.label,w.name].filter(Boolean).join(' '); const isVideo=VIDEO_HINT.test(`${mediumText} ${pointerText} ${titleText}`);
  ...
  if(isVideo){
   stats.video.works++; roots.forEach(x=>inc(stats.video.byRoot,x)); inc(stats.video.byPeriod,period); normalized.forEach(x=>inc(stats.video.byLanguage,x)); rawMedium.forEach(x=>inc(stats.video.byMedium,x));
   if(stats.video.cinemaCandidates.length<5000)stats.video.cinemaCandidates.push({id:w.id||w.workId||w.canonicalId||null,title:w.title||w.label||w.name||null,languages:normalized,period,roots,medium:rawMedium,pointers});
  }
 }
}
