...
const context={console,globalThis:null,fetch:async()=>({ok:true,json:async()=>({schema:'dore.browser-search-core.v1',verses:refs})})};
...
(async()=>{
  ...
  const w=await D.conceptSearch('曠野裡耶穌受試探和申命記六到八章有什麼關係？');
  ...
  const s=await D.conceptSearch('為什麼舊約裡沒有聖靈？');
