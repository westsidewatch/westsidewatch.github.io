...
REPORT_PATH = Path("reports/DORÉ-CORPUS-READING-REPORT.json")
...
def main() -> None:
    reports, total_tokens, total_warnings, total_failures = {}, 0, 0, 0
    try:
        for code, filename in OT_TARGETS.items():
            ...
            total_tokens += report.emitted_tokens; total_warnings += report.warnings; total_failures += report.failures
        ...
        result = {"status":"PASS","books_expected":66,"books_read":len(reports),"tokens_read":total_tokens,"warnings":total_warnings,"failures":total_failures,"silent_token_loss":0,"snapshots":{"oshb":OSHB_SHA,"morphgnt_sblgnt":MORPHGNT_SHA},"reports":reports}
    except Exception as exc:
        result = {"status":"FAIL","books_expected":66,"books_read":len(reports),"tokens_read":total_tokens,"warnings":total_warnings,"failures":total_failures + 1,"error":f"{type(exc).__name__}: {exc}","snapshots":{"oshb":OSHB_SHA,"morphgnt_sblgnt":MORPHGNT_SHA},"reports":reports}
        REPORT_PATH.parent.mkdir(parents=True, exist_ok=True); REPORT_PATH.write_text(json.dumps(result, ensure_ascii=False, indent=2, sort_keys=True), encoding="utf-8")
