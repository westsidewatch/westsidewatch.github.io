...
assert "windowCards=84" in runtime and "releaseFarFrom" in runtime
assert "prefetchAhead=1" in js and "TRACKS=[" in js
...
# Product must never make classification/editorial layers admission authority.
for forbidden in ('admissionAuthority=true','admissionAuthority: true','admissionAuthority = true'):
    assert forbidden not in js
# Static DOM contract: no code path requests or renders the full canonical count.
