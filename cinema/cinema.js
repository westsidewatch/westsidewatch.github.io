...
function sourceFor(item){return (item.sources||item.providerSources||[])[0]||{};}
...
function playbackTarget(item,startMs=0){return window.HolyLightProviders?.resolve(item,seconds(startMs))||null;}
function openAt(item,startMs=0){
  if(item.canonicalId===JESUS_ID){const inline=document.querySelector('.living-poster__media');const shell=document.querySelector('.living-poster');if(inline&&shell){mountInlinePlayer(shell,inline,item,startMs);return;}}
...
function mountInlinePlayer(shell,target,item,startMs=0){
  ...
  if(!mounted){const fallback=playbackTarget(item,startMs);if(fallback?.kind==='handoff')window.open(fallback.url,'_blank','noopener,noreferrer');return;}
  shell.dataset.state='playing';shell.classList.add('is-playing');const close=shell.querySelector('[data-action="close-player"]');close.hidden=false;document.documentElement.dataset.cinemaPlayback='inline';
}
function closeInlinePlayer(shell){const target=shell.querySelector('.living-poster__media');target?.replaceChildren();shell.classList.remove('is-playing');shell.dataset.state='focus';const close=shell.querySelector('[data-action="close-player"]');if(close)close.hidden=true;delete document.documentElement.dataset.cinemaPlayback;}
...
function renderLivingPoster(item){
  ...
  const list=shell.querySelector('ol');JESUS_CHAPTERS.forEach((label,index)=>{const li=document.createElement('li');li.innerHTML=`<span>${String(index+1).padStart(2,'0')}</span><strong>${label}</strong>`;list.appendChild(li);});
  let focusTimer;const focus=()=>{clearTimeout(focusTimer);focusTimer=setTimeout(()=>{if(!shell.classList.contains('is-playing')&&shell.dataset.state!=='expand')shell.dataset.state='focus';},320);};const settle=()=>{clearTimeout(focusTimer);if(!shell.classList.contains('is-playing')&&shell.dataset.state!=='expand')shell.dataset.state='settle';};
